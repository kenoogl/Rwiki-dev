...
class TestBuildFrontmatter:
  ...
  def test_dict_to_yaml(self):
    ...
    result = rw_utils.build_frontmatter(meta)
  ...
  def test_roundtrip_with_special_chars(self):
    ...
    built = rw_utils.build_frontmatter(original)
    parsed, body = rw_utils.parse_frontmatter(built + "body")
...
class TestListMdFiles:
  ...
  def test_returns_only_md_files(self, tmp_path):
    ...
    result = rw_utils.list_md_files(str(tmp_path))
  ...
  def test_nonexistent_dir_returns_empty(self, tmp_path):
    ...
    result = rw_utils.list_md_files(str(tmp_path / "does_not_exist"))
    assert result == []
...
class TestFirstH1:
  ...
  def test_h1_found(self):
    ...
    text = "## Section\n\n# My Title\n\nBody"
    assert rw_utils.first_h1(text) == "My Title"

  def test_no_h1(self):
    """H1 見出しがない場合は None を返す"""
    text = "## Section\nJust body text without H1"
    assert rw_utils.first_h1(text) is None
...
class TestEnsureBasicFrontmatter:
  ...
  def test_missing_fields_are_filled(self, tmp_path, fixed_today):
    ...
    modified, fixes, new_text = rw_utils.ensure_basic_frontmatter(str(path), "web")
    ...
    meta, body = rw_utils.parse_frontmatter(new_text)
  ...
  def test_missing_title_uses_basename_when_no_h1(self, tmp_path, fixed_today):
    ...
    modified, fixes, new_text = rw_utils.ensure_basic_frontmatter(str(path), "local")
  ...
  def test_all_fields_present_no_change(self, tmp_path, fixed_today):
    ...
    body = "Body text"
    content = rw_utils.build_frontmatter(meta) + "\n" + body + "\n"
    path = tmp_path / "existing.md"
    path.write_text(content, encoding="utf-8")

    modified, fixes, new_text = rw_utils.ensure_basic_frontmatter(str(path), "unknown")
...
class TestFileIO:
  ...
  def test_write_and_read_japanese_utf8(self, tmp_path):
    ...
    rw_utils.write_text(str(path), content)
    result = rw_utils.read_text(str(path))
    assert result == content

  def test_append_text_japanese(self, tmp_path):
    """append_text で日本語テキストを追記でき、既存内容が保持される"""
    path = tmp_path / "append_test.md"
    rw_utils.write_text(str(path), "最初の行\n")
    rw_utils.append_text(str(path), "追記した行\n")
    result = rw_utils.read_text(str(path))
  ...
  def test_write_creates_parent_dirs(self, tmp_path):
    ...
    rw_utils.write_text(str(path), "content")
    assert path.exists()
    assert rw_utils.read_text(str(path)) == "content"
...
class TestReadJson:
  ...
  def test_valid_json_returns_dict(self, tmp_path):
    ...
    result = rw_utils.read_json(str(path))
