...
- 所見の四要素（`discipline_finding_4elements.md`）— 事実を残す場面の観点規律。各役の出力をレビュー記録ファイルに書き出すときは、本ファイルが定める四要素（箇所・現状・問題・修正後）に従う。
- レビュー所見のテンプレート（`discipline_review_log_template.md`）— 利用者に判断を仰ぐ場面の形式規律。記録ファイルから利用者向けメッセージを組み立てるときに参照する。
