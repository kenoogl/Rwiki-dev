...
- 重要な構造的決定の事前確認（`feedback_ssot_structural_decision_check.md`）— 同種の自己チェック規律。本ファイルは「不整合発見時の即解消提案抑制」、対する側は「構造的決定の運用前の暗黙デフォルト採択抑制」を扱う。両者は対象が違うが、エージェントの即決偏向を抑える点で対になる。
