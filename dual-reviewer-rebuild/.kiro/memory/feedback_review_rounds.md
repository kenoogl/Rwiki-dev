...
2. **網羅的 grep 検索**: 各変更値について、以下の対象を grep で参照箇所を特定
   - 既 approve 済の他 spec 全件 (`.kiro/specs/rwiki-v2-*/requirements.md`、当該 spec を除く)
   - drafts (`.kiro/drafts/rwiki-v2-consolidated-spec.md` / `rwiki-v2-scenarios.md`)
   - Foundation (`.kiro/specs/rwiki-v2-foundation/requirements.md`)
...
4. **Adjacent Sync TODO 整理**: 文言同期が必要な箇所を Adjacent Sync TODO として記録 (D-N 通し番号)。各 TODO は (a) 対象 spec / drafts、(b) 修正前後の文言、(c) 同期理由 (本セッション spec の修正由来) を記載
5. **本セッション内同期判断**: 各 Adjacent Sync TODO について、本セッション内で同期適用するか別セッションに残すかをユーザー判断する。判断材料は (a) 既 approve spec か未 approve spec か、(b) 文言同期レベル (再 approval 不要) か実質要件変更か、(c) 関連 spec の現状
...
各ラウンドで発見した修正候補は `feedback_deepdive_autoadopt.md` の方針に従って深掘り検討し、自動採択 / escalate を判断する。escalate 条件 5 種に該当する場合は必ずユーザー判断を仰ぐ。
...
- `feedback_deepdive_autoadopt.md`: 各ラウンドで発見した修正候補の深掘り検討 + 自動採択方針
