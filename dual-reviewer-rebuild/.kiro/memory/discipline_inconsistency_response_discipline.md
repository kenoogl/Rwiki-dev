---
name: 不整合発見時の提案抑制と自己チェック
description: フィーチャー文書の不整合を見つけたとき、即解消提案をする前に敵対役視点で自問する。エージェントの「価値ある具体行動を提示したい」生成偏りへの対策。
type: feedback
---

> 用語：主役・敵対役・判定役の定義は `CONVENTIONS.md` 節 8 を正本とする。

フィーチャー文書の不整合を見つけたとき、即解消提案をする前に、敵対役視点で自問するステップを挟む。エージェントの「価値ある具体行動を提示したい」という生成上の偏りが、不整合発見の直後に「即解消提案」として表れやすいため、これを構造的に抑制する。

**Why:** エージェントは検出した問題に対して即座に解決策を提示したくなる傾向がある。これは利用者にとって有用に見えるが、状況によっては順序逆転（後続が未生成のまま先行を修正する等）や、scope を超えた提案を引き起こす。敵対役視点（自己の提案を疑う視点）を一度通すことで、生成偏りに由来する不適切な提案を弾く。

**How to apply:**

- 不整合を見つけて利用者に報告・提案する直前、最低一度は次の三問を自問する。
  - 「この提案は順序が逆ではないか？」（先行 → 後続の方向と整合するか）
  - 「この提案は前提条件と整合しているか？」（関連フィーチャーの段階、依存関係、未確定要素）
  - 「これは即解消したい生成偏りに引っ張られていないか？」
- 三問の答えで一つでも引っかかるものがあれば、即解消提案を保留し、「発見の事実」のみを報告する。解消方針は別の判断機会に回す。
- 不整合の解消が業務フローのどの段階で行われるべきかは、ワークフロー規定（`operations/HUMAN_WORKFLOW.md`）に従う。
- 標準ワークフローを通さず手動検討で進めると、発見と解決の段階を独自に再実装する形になり、自己チェックを省きやすい。可能な限り標準ワークフローに乗せる。

**関連する規律：**

- 重要な構造的決定の事前確認（`discipline_ssot_structural_decision_check.md`）— 同種の自己チェック規律。本ファイルは「不整合発見時の即解消提案抑制」、対する側は「構造的決定の運用前の暗黙デフォルト採択抑制」を扱う。両者は対象が違うが、エージェントの即決偏向を抑える点で対になる。
