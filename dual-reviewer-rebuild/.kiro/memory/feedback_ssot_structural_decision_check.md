...
- 不整合発見時の提案抑制と自己チェック（`feedback_inconsistency_response_discipline.md`）— 同種の自己チェック規律。本ファイルは「構造的決定の運用前の暗黙デフォルト採択抑制」、対する側は「不整合発見時の即解消提案抑制」を扱う。両者は対象が違うが、エージェントの即決偏向を抑える点で対になる。
