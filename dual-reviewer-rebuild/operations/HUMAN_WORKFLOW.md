...
レビューの段階別に適用すべき基本規律（検出 / 提示 / 判定 / 記録）と、各レビュー段階（要件 / 設計 / 横断 / タスク）の詳細は [`REVIEW_PROTOCOL.md`](REVIEW_PROTOCOL.md) を参照。
...
判断軸は 規律 `discipline_design_spec_roundtrip.md` を参照する。判断の結果に応じて次の手順を採る。
...
4. 利用者の明示承認を得てから承認状態を更新する（規律 `discipline_approval_required.md` 参照）

#### 設計内吸収が確定した場合

1. design.md 内に「設計決定事項」として記録する（規律 `discipline_design_decisions_record.md` 参照）
