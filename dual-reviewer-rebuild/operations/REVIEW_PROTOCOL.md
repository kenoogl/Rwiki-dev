...
本文書は独自の節番号（節 1〜5）を用いる。`HUMAN_WORKFLOW.md` 節 5.2.8 / 5.2.9 / 5.2.10 / 5.2.11 はプレースホルダのみが残り、そこから本文書の該当節へ 2 段リンクで対応する（HUMAN_WORKFLOW.md 5.2.8→本文書 節 2、5.2.9→節 3、5.2.10→節 4、5.2.11→節 5）。規律ファイル群は HUMAN_WORKFLOW.md 側の 5.2.x を参照するため、本文書の番号変更の影響を受けない。

用語：主役・敵対役・判定役の定義は `CONVENTIONS.md` 節 8 を正本とする。本文書はレビュー手続きの正本であり、用語定義の正本ではない。

---

## 1. レビュー基本規律（全レビュー段階で適用）
...
要件レビューの 5 ラウンド構成(節 2)を設計に流用すると、設計特有の観点(アーキテクチャ整合、性能達成手段、失敗時の処理の具体化、観測性など)が不足する。要件レビューで確定済みの内部矛盾や唯一の正本整合は、設計時には再検証コストが低いため割愛し、代わりに設計特有の観点を網羅すべきである。設計フェーズはラウンド数を要件レビューの 5 から増やし、10 観点 = 10 ラウンドとして全ラウンドを基本実施する。
...
要件レビュー（節 2）と設計レビュー（節 3）はそれぞれ「何を満たすか」「どう実現するか」の検査だったのに対し、タスクレビューは「どの順序でどの単位で実装するか」を検査する。観点は実装直前固有の問題（粒度、依存、検証可能性）に絞る。
...
- **横断タスクは中心フィーチャー側に置く**：複数フィーチャーにまたがる作業は、設計レビューの「中心フィーチャー判定」（節 3）と同じ判定で置き場所を決める
