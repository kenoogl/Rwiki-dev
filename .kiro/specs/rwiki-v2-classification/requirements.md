...
7. The Classification System shall `review/decision-views/*`（Decision log の Tier 2 markdown timeline 出力先、`_candidates` 命名規則の対象外）の frontmatter 骨格として `decision_id`（参照元 decision ID、必須）/ `tier`（固定値 `2`、Decision visualization Tier marker、必須）/ `period_start`（YYYY-MM-DD、timeline 期間の開始、必須）/ `period_end`（YYYY-MM-DD、timeline 期間の終了、必須）/ `entity`（対象 entity または subject ref、任意）/ `source_decisions`（元になった decision_id の配列、任意）/ `generated_at`（YYYY-MM-DD、生成日時、必須）の 7 field を宣言し、render 実装と出力トリガは Spec 5 の `rw decision render` 等が担うことを明示する（Foundation Requirement 8.4 の Spec 1 委譲を引き受ける）。
...
1. The Classification System shall `.rwiki/vocabulary/tags.yml` の最小スキーマとして、各タグエントリに `canonical`（文字列、unique、必須）/ `description`（文字列、任意）/ `aliases`（文字列配列、任意）/ `deprecated`（真偽、デフォルト false）/ `deprecated_reason`（文字列、`deprecated: true` 時に推奨）/ `successor_tag`（後継 canonical タグ名、文字列または文字列配列、`deprecated: true` 時に推奨、後継なしの純粋廃止では省略可。**wiki frontmatter `successor:` (path 配列) との命名衝突を回避するため `successor` ではなく `successor_tag` を採用** — 設計決定 1-4 由来）の 6 項目を宣言可能にする。
...
4. While Spec 3 が dispatch 実装を行う段階において, the Classification System shall 自身が field の意味と vocabulary 表のみを所管し、dispatch 優先順位（明示 → `type:` → category default → `applicable_input_paths` glob match → LLM 判断、Spec 3 Requirement 1 の 5 段階優先順位、段階 3.5 は Spec 2 Requirement 3.2 由来 coordination として 2026-04-27 確定）の **判断ロジック自体は Spec 3 が所管** することを明示する。
...
- 2026-04-27 (Adjacent Sync): Spec 3 第 5 ラウンド由来 — R11.4 dispatch 優先順位記述を 4 段階 → 5 段階化（明示 → `type:` → category default → `applicable_input_paths` glob match → LLM 判断）。段階 3.5（`applicable_input_paths` glob match）は Spec 2 Requirement 3.2 由来 coordination で 2026-04-27 確定、本 spec は dispatch 判断ロジックの所管 spec が Spec 3 である事実のみ規定する立場を維持し、新段階追加は Spec 3 Requirement 1 を SSoT として参照する。Adjacent Spec Synchronization 運用ルールに従い再 approval は不要、`spec.json.updated_at` 更新のみ。
- 2026-04-27 (実質変更): Spec 1 design phase 12 ラウンドレビュー由来の設計決定 1-4 / 1-13 / 1-16 を本 requirements に反映 (3 件一括改版)。R6.1 tags.yml field 名 `successor` → `successor_tag` rename (D-3 命名衝突回避、決定 1-4) / R4.7 `review/decision-views/*` の `period_start` / `period_end` を任意 → 必須化 (Tier 2 timeline schema 明確化、決定 1-13) / R9.1 lint 検査項目に「許可値外 `merge_strategy:` WARN」を追加 (設計持ち越し L 由来、決定 1-16)。実質変更経路のため再 approval を伴う。
