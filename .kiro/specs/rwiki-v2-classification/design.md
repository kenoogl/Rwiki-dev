...
- `rw tag *` 12 サブコマンドの動作仕様を確定し、Spec 4 CLI 統一規約と整合
...
- **`rw tag *` 12 サブコマンド**: scan / stats / diff / merge / split / rename / deprecate / register / vocabulary {list, show, edit} / review (R8)
- **lint vocabulary 統合**: 12 検査項目 (未登録タグ / エイリアス / 非推奨 / 未登録カテゴリ / 許可値外 type / 許可値外 entity_type / shortcuts 不整合 / vocabulary 重複) + severity 4 水準 (R9)
...
- **Python 標準ライブラリ**: `unicodedata` (NFC normalization) / `pathlib` (file walking) / `difflib` (Ratcliff/Obershelp 類似度 による suggestion) / `sqlite3` (Phase 2 cache、決定 1-8)
...
- **Selected pattern**: Module-per-Domain (例外 1 件) — G1-G6 sub-system のうち、G1+G2 を `rw_classification.py` に統合 (G1 は 1 関数のため独立 module 化を避ける、frontmatter parse 経由の密結合理由)、G3-G5 を別 module (`rw_vocabulary.py` / `rw_tag.py` / `rw_lint_vocabulary.py`)、G6 (Coordination) は cross-cutting 概念で独立 module ファイルを持たず Coordination セクション (G6) で 6 coordination を集約。共通 utility は `rw_utils.py` (Spec 4 設計決定 4-3 で v2 新規実装) を Spec 1 implementation で拡張 (research.md Architecture Pattern Evaluation 採択)
- **Domain/feature boundaries**: G1 カテゴリ / G2 frontmatter / G3 vocabulary / G4 CLI / G5 lint / G6 coordination の 6 sub-system が責務分離。実装配置は **4 module (G1+G2 統合, G3, G4, G5) + cross-cutting (G6) + Spec 4 共通 utility (`rw_utils.py`)**
...
|-------|------------------|-----------------|-------|
| CLI Runtime | Python 3.10+ | `rw tag *` 12 サブコマンド + lint vocabulary 統合 | プロジェクト標準 (steering tech.md) |
...
├── rw_tag.py                # G4 rw tag CLI 12 サブコマンドのエントリ
├── rw_lint_vocabulary.py    # G5 lint vocabulary 統合の 12 検査項目 + 決定 1-16 拡張項目
└── rw_utils.py              # Spec 4 で v2 新規実装される共通 utility (Severity / exit code / timeout helper / argparse factory、Spec 4 設計決定 4-3 で 9 module 分割確定済、Phase 順序により先行作成の可能性)、本 spec implementation で NFC normalization 関数 normalize_token を追加 (決定 1-3)
...
> - `rw_tag.py` = `rw tag *` 12 サブコマンドのエントリポイント (subprocess timeout 必須)
> - `rw_lint_vocabulary.py` = lint 12 検査項目 + suggestion 生成 (Ratcliff/Obershelp 類似度)
> - `rw_utils.py` = Spec 4 で v2 新規実装される共通 utility に NFC normalization / file walking 共通 helper を追加 (Spec 4 設計決定 4-3 と整合)
...
flowchart LR
  ...
  WalkPages --> CheckEach[各 page で 12 検査項目]
...
| 8.1-8.14 | rw tag * 12 サブコマンド | G4 + G6 coordination | CLI signature + Spec 4 lock + Spec 5 record_decision | Flow 1 |
...
- Validation: 12 検査項目を `LintFinding` として集約、severity 4 水準
...
**Contracts**: State ✓ (vocabulary YAML 3 種は永続状態) + Service ✓ (load/query/dump メソッド群)

##### Service Interface

```python
class VocabularyStore:
    def load(self, vault_path: Path) -> "VocabularyStore":
        """
        Load tags.yml / categories.yml / entity_types.yml from <vault>/.rwiki/vocabulary/.
        未配置時は空の VocabularyStore を返却 (fail-soft、R9.4)。
        parse 失敗時は VocabularyStore(parse_errors=...) を返却 (R9.7)。
        """

    def get_canonical(self, token: str, vocab_type: Literal["tag", "category", "entity_type"]) -> Optional[str]:
        """Return canonical for the given token, or None if not registered."""

    def is_alias(self, token: str, vocab_type: Literal["tag", "category", "entity_type"]) -> bool:
        """Return True if the token is an alias of any canonical."""

    def is_deprecated(self, canonical: str, vocab_type: Literal["tag", "category", "entity_type"]) -> bool:
        """Return True if the canonical is marked deprecated: true (R6.1 successor_tag, 決定 1-4)."""

    def get_entity_type(self, name: str) -> Optional[EntityType]:
        """
        Return EntityType (name / description / directory_name / shortcuts mapping) or None.
        directory_name は決定 1-9 で追加された任意 field、shortcuts.<name>.directed は決定 1-12 (任意、デフォルト directed=true)。
        """

    def normalize_token(self, s: str) -> str:
        """Apply NFC normalization (rw_utils.normalize_token に委譲、決定 1-3)."""

    def dump(self, vocab_type: Literal["tag", "category", "entity_type"], data: dict) -> None:
        """
        Persist vocabulary YAML with .hygiene.lock acquired (write 系操作必須、決定 1-14)。
        write 前に NFC 正規化 / 重複検出 / YAML injection 防御を適用 (B-5、決定 1-3)。
        """
```

- Preconditions: `<vault>/.rwiki/vocabulary/` 配下に YAML 3 種が配置済 (`rw init` 実行後) または未配置
- Postconditions: load 成功時 VocabularyStore は in-memory 状態として 3 vocabulary を保持、未配置時は空 VocabularyStore (R9.4 整合)
- Invariants: read 系メソッド (`get_canonical` / `is_alias` / `is_deprecated` / `get_entity_type` / `normalize_token`) は thread-safe (file read のみ、in-memory immutable)、write 系 (`dump`) は `.hygiene.lock` 取得必須でシリアライズ (R8.14、決定 1-14)
- Error model: YAML parse failure 時 `VocabularyStore(parse_errors=List[VocabError])` を返却、例外を投げない (fail-soft、R9.7)。caller (G5 lint) は `parse_errors` を ERROR finding として扱う。Phase 2 Spec 4 design 着手時に内部 helper メソッド (`_validate_schema` / `_load_yaml_file` 等) の signature 詳細化を coordination
...
- **12 検査項目** (R9.1):
  ...
  - **追加: 未使用カテゴリ INFO** (B-16 由来、決定 1-17 で確定、`categories.yml` に登録済だが対応 wiki/<category>/ ディレクトリが空 or 存在しない、要件改版経路 = R9.1 改版に統合)
...
entity_types:
  - name: string              # 必須、unique (kebab case 英語推奨、日本語も許容 = B-10)
    ...
    shortcuts:                # mapping (shortcut field 名 → 属性、決定 1-12 access pattern と整合、Spec 5 normalize_frontmatter API は dict access 想定)
...
added: 2026-04-27
entity_type: entity-person     # 別 field、R2.7 で type と直交分離 (本-2 / 本-15 由来、決定 1-3 = NFC normalization 規約とは別経緯)
...
- lint 12 検査項目 + suggestion (Ratcliff/Obershelp 類似度 ratio ≥ 0.8)
- `rw tag *` 12 サブコマンドの個別 test (mock LLM CLI)
...
**Transaction Guarantee Integration Tests** (Phase 1 旧 R7「Eventual Consistency」escalate 対応、新 10 ラウンド構成では R6 失敗+観測性に統合):
...
- **Subprocess shell injection 防止**: `rw tag merge` / `split` 等で LLM CLI を `subprocess` 経由で呼び出す際 (R8.12 timeout 必須と整合)、Phase 2 Spec 4 design 実装では `shell=False` 必須、引数を args list 形式で渡し、user 入力 (canonical / aliases / vault path 等) を直接 shell に渡さない。Special characters 混入 (例: `; ` / `&&` / `$()` / バックティック) でも injection を防ぐ。Spec 1 では規範のみ宣言、実装は Spec 4 で確定。Spec 0 design Security Considerations「Subprocess shell injection 防止」と同等規律
...
### 本 design 確定後の連鎖 migration (4 件、実質変更経路)

本 design 内で確定した決定が requirements.md 改版を伴う (実質変更経路、別 commit で再 approve)。Spec 1 やり直しラウンド 1 (2026-04-28) で 3 件の規範範囲先取りを発見、既存 R9.1 改版に統合 + R5.2 新規改版 1 件を追加して計 4 改版経路:

- **決定 1-4** (`successor` → `successor_tag` rename): requirements.md R6.1 改版
- **決定 1-13** (`decision-views/` `period_start` / `period_end` 必須化): requirements.md R4.7 改版
- **決定 1-16 + 決定 1-3 + 決定 1-17** (lint G5 検査項目拡張): requirements.md R9.1 改版に「許可値外 `merge_strategy:` WARN」(決定 1-16) +「全角半角混在 WARN」(重-1-1 / 決定 1-3 由来) +「未使用カテゴリ INFO」(重-5-2 / 決定 1-17 由来) を統合追加。R9.1 元 10 項目 → 12 項目 に拡張
- **決定 1-9 + 決定 1-12** (entity_types.yml schema 拡張): requirements.md R5.2 改版で「3 項目 → 任意拡張表現」+「shortcuts に `directed` 任意 field 含む」を統合明示 (重-1-2 / 重-1-3 やり直しラウンド 1 由来、`directory_name` / `directed` の規範範囲を要件レベルで明示)

4 改版経路は本 design approve 後の **連鎖 migration として位置付け**、1 commit に集約 + 再 approve PR 起票 (前例 = Phase 1 完了時の決定 1-4 / 1-13 / 1-16 と同手法)。

### vocabulary YAML 3 種の初期化 migration

`rw init` で `<vault>/.rwiki/vocabulary/` 配下に空 YAML 3 種を生成 (R6.6)。**初期 seed 規範**:

- `categories.yml`: 推奨 6 カテゴリ (`articles` / `papers` / `notes` / `narratives` / `essays` / `code-snippets`、enforcement = `recommended`、R1.1 / R7.1 整合) を初期 entry として seed
...
- **Adjacent Sync 範囲** (文言整合のみ、要件改版を伴わない) の例: foundation.md §5 骨格への注記追加 (決定 1-15、vocabulary entry の §2.10 / §2.13 適用範囲) や用語 typo 訂正、章節アンカーの追加など。技術的後方互換性 + 規範範囲拡張なし の組み合わせ
- **実質変更経路** (要件改版 + 再 approve 必須) の例:
  - (a) **後方互換 (任意 field 追加) でも規範範囲先取りの場合**: 決定 1-9 `entity_types.yml.directory_name` 追加 (markdown 影響なしの技術的後方互換だが、requirements R5.2 で「3 項目を宣言可能」と固定列挙されており規範範囲拡張、重-1-2 で R5.2 改版経路化済)
  - (b) **後方非互換 (必須 field 追加 / 値域変更) の場合**: 将来 `tags.yml.canonical_kind` 必須化 = 既存 markdown migration script が必要 + R6.1 改版
- **判定軸**: Adjacent Sync vs 実質変更経路の判定は「**規範範囲拡張の有無**」で決定。技術的後方互換性 (markdown 影響有無) は独立軸、後方互換でも規範範囲先取りなら実質変更経路
...
- **影響**: vocabulary store 入力時に `unicodedata.normalize('NFC', s)` 適用、lint G5 で全角半角混在を WARN 検出。requirements.md R9.1 改版 (lint 12 検査項目に「全角半角混在 WARN」追加、実質変更経路、別 commit 再 approve、決定 1-16 と同 commit に統合) — Spec 1 やり直しラウンド 1 (2026-04-28、重-1-1) で規範範囲先取りを解消
- **Follow-up**: 本 design approve 後、Spec 1 requirements.md R9.1 改版 PR を起票 (決定 1-16 と統合)
...
- **影響**: G3 entity_types.yml schema に 1 field 追加、後方互換 (任意 field)、foundation.md §5 骨格にも反映 (Adjacent Sync)。requirements.md R5.2 改版 (「3 項目を宣言可能」→「3 項目 + 任意拡張 (`directory_name` 等) を宣言可能」、実質変更経路、別 commit 再 approve、決定 1-12 と同 commit に統合) — Spec 1 やり直しラウンド 1 (2026-04-28、重-1-2) で規範範囲先取りを解消
- **Follow-up**: 本 design approve 後、Spec 1 requirements.md R5.2 改版 PR を起票 (決定 1-12 と統合)
...
- **影響**: Phase 3 Spec 5 design で `relations.yml` 初期 typed edge 集合 (`authored` / `collaborated_with` / `mentored` / `implements` 含む) 確定。requirements.md R5.2 改版 (`shortcuts` mapping に `directed` 任意 field を含む明示、実質変更経路、別 commit 再 approve、決定 1-9 と同 commit に統合) — Spec 1 やり直しラウンド 1 (2026-04-28、重-1-3) で規範範囲先取りを解消
- **Follow-up**: 本 design approve 後、Spec 1 requirements.md R5.2 改版 PR を起票 (決定 1-9 と統合)
...
### 決定 1-17: lint G5 検査項目に「未使用カテゴリ INFO」を追加 (要件改版経路、Spec 1 やり直しラウンド 5 重-5-2 由来)

- **経緯**: 設計持ち越し B-16、`categories.yml` に登録済だが対応 wiki/<category>/ ディレクトリが空 or 存在しない vocabulary curation UX 課題。Phase 1 design 初版で「追加: 未使用カテゴリ INFO」として記述したが R9.1 lint 検査項目への明示反映が漏れて規範範囲先取り状態だった (Spec 1 やり直しラウンド 5 重-5-2 で発見)
- **決定**: lint G5 に「未使用カテゴリ INFO (`categories.yml` に登録済 name のうち、対応 wiki/<category>/ ディレクトリが空 or 存在しない場合)」検査項目を追加。requirements.md R9.1 改版を伴うため別 commit で実質変更経路、決定 1-3 (全角半角 WARN) / 決定 1-16 (許可値外 merge_strategy WARN) と同 commit に統合
- **影響**: 本 design 確定後、Spec 1 requirements.md R9.1 改版 PR を起票 (決定 1-16 + 決定 1-3 + 決定 1-17 統合)、再 approve。Spec 0 R4 escalate「規範範囲先取り削除」と同型問題を解決方針 (要件改版経路) で整合化
- **Follow-up**: 本 design approve 後、Spec 1 requirements.md R9.1 改版 PR を起票 (決定 1-16 + 決定 1-3 + 決定 1-17 統合)
...
- 2026-04-28: Spec 1 やり直しラウンド 1 (requirements 全 AC の網羅) — 新方式 4 重検査 + Step 1b-v 自動深掘り 5 切り口 + 厳しく検証 default + negative 視点強制発動 適用、計 4 件確定:
  - **重-1-1** (要件改版経路、決定 1-3 関連): NFC + 全角半角 WARN を requirements.md R9.1 改版経路化 (lint 12 検査項目に「全角半角混在 WARN」明示追加、規範範囲先取り解消)、既存 R9.1 改版 (決定 1-16 = 許可値外 `merge_strategy:` WARN) と同 commit に統合
  - **重-1-2** (要件改版経路、決定 1-9 関連): `entity_types.yml.directory_name` 任意 field を requirements.md R5.2 改版経路化 (3 項目固定 → 任意拡張表現、規範範囲先取り解消)
  - **重-1-3** (要件改版経路、決定 1-12 関連): `shortcuts.<name>.directed` 任意 field を requirements.md R5.2 改版経路化 (shortcuts mapping に `directed` 任意 field 含む明示、規範範囲先取り解消)、決定 1-9 と同 commit に統合
  - **重-1-4** (design 内修正): G3 VocabularyStore に `##### Service Interface` セクション追加 (`load` / `get_canonical` / `is_alias` / `is_deprecated` / `get_entity_type` / `normalize_token` / `dump` の class methods signature 明示)、Spec 0 R2 重-厳-3 と同型構造的不均一を修正
  - 連鎖 migration セクションを 3 件 → 4 改版経路に拡張 (R6.1 / R4.7 / R9.1 統合 / R5.2 統合)、本 design approve 後の連鎖 migration として位置付け維持

requirements.md 改版: 本ラウンドで新規確定 (R9.1 全角半角 WARN + R5.2 任意拡張 + R5.2 directed)、本 design approve 後の連鎖 migration で 1 commit に集約

- 2026-04-28: Spec 1 やり直しラウンド 2 (アーキテクチャ整合性) — 新方式 4 重検査 + Step 1b-v 自動深掘り 5 切り口 + 厳しく検証 default + negative 視点強制発動 適用、計 3 件適用 (要件改版なし、design 内修正のみ):
  - **軽-2-1** (自動採択、design 内修正): File Structure Plan の `rw_utils.py` 説明を「既存共通 utility」→「Spec 4 で v2 新規実装される共通 utility (Severity / exit code / timeout helper / argparse factory、設計決定 4-3 で 9 module 分割確定済、Phase 順序により先行作成の可能性)、本 spec implementation で normalize_token を追加」へ整合化、L210 の rw_utils.py 説明も同等に整合化
  - **軽-2-2 + 重-2-1** (escalate 確証手順、design 内修正): Architecture Integration L131-135 段落書き換え — Selected pattern 行に「G1+G2 統合 (G1 1 関数のため過剰分割回避) / G3-G5 別 module / G6 cross-cutting (Coordination セクションで集約) / rw_utils.py Spec 4 拡張」を明示、Domain/feature boundaries 行に「実装配置は 4 module + cross-cutting (G6) + Spec 4 共通 utility (rw_utils.py)」追記。L131「G1-G6 を別 module で構成」vs L168「G1+G2 統合」の文言矛盾を解消、Spec 0 R2 重-厳-3 と同型構造説明欠落修正

requirements.md 改版: なし (本ラウンドはすべて design 内修正、連鎖 migration への新規追加なし)

- 2026-04-28: Spec 1 やり直しラウンド 3 (データモデル / スキーマ詳細) — 新方式 4 重検査 + Step 1b-v 自動深掘り 5 切り口 + 厳しく検証 default + negative 視点強制発動 適用、計 3 件適用 (要件改版なし、design 内修正のみ):
  - **軽-3-1** (自動採択、design 内修正): L3 wiki frontmatter active example の `update_history` type コメントを「extension / refactor / deprecation / merge」 (4 値) → 「synthesis 起源: extension / refactor、lifecycle 起源: deprecation / merge / split / archive (R3.6)」 (6 値) へ整合化
  - **軽-3-2** (自動採択、design 内修正): Entity-person example のコメント「決定 1-3 で type と直交分離」が誤記 (決定 1-3 は NFC normalization 規約)、「R2.7 で type と直交分離 (本-2 / 本-15 由来、決定 1-3 = NFC normalization 規約とは別経緯)」へ訂正
  - **重-3-1** (escalate 確証手順、design 内修正): entity_types.yml schema 例 L866-874 を **list of objects 表現** → **mapping 表現** に修正 (`shortcuts: <field_name>: { typed_edge, directed }`)、design 本文 L495 / L831 / 決定 1-12 access pattern (`entity-person.shortcuts.authored.directed`) および Spec 5 normalize_frontmatter API の dict access 想定と整合化。Phase 1 Spec 1 R5 escalate (Levenshtein → Ratcliff/Obershelp 文書 vs 実装不整合) と同型問題、解決方針 (文書側訂正) 整合

requirements.md 改版: なし

- 2026-04-28: Spec 1 やり直しラウンド 4 (API interface 具体化) — 新方式 4 重検査 + Step 1b-v 自動深掘り 5 切り口 + 厳しく検証 default + negative 視点強制発動 適用、計 1 件適用 (要件改版なし、design 内修正のみ):
  - **重-4-1** (escalate 確証手順、design 内修正): design.md 全 10 箇所「13 サブコマンド」を「12 サブコマンド」に一括修正 (R8.1 列挙整合、L5 / L15 / L40 / L141 / L170 / L208 / L326 / L576 / L587 / L1043)。Spec 0 R7-R10 重-厳-5 (approvals.requirements 誤記 2 箇所) と同型問題 (単純誤記 grep 検出、複数箇所一括修正)。requirements / brief / consolidated-spec には「13」表記なし、design.md 単独誤記が確実

requirements.md 改版: なし

- 2026-04-28: Spec 1 やり直しラウンド 5 (アルゴリズム + 性能達成手段) — 新方式 4 重検査 + Step 1b-v 自動深掘り 5 切り口 + 厳しく検証 default + negative 視点強制発動 適用、計 2 件適用 (連動、要件改版 1 件 + design 内修正 1 件):
  - **重-5-2** (要件改版経路、escalate 確証手順、新規 決定 1-17): R9.1 改版経路化「未使用カテゴリ INFO」検査項目追加 (B-16 由来、Phase 1 design 初版で「追加」として記述したが R9.1 反映漏れの規範範囲先取り解消)、決定 1-16 + 決定 1-3 + 決定 1-17 を R9.1 改版で同 commit に統合、R9.1 元 10 項目 → **12 項目** に拡張
  - **重-5-1** (escalate 確証手順、design 内修正): design 全 7 箇所「8 検査項目」を「12 検査項目」に一括修正 (L41 / L171 / L209 / L257 / L474 / L647 / L1042、R9.1 改版後 12 項目に対応)。Spec 0 R7-R10 重-厳-5 と同型問題 (単純誤記 grep 検出 + 複数箇所一括修正)
  - 連鎖 migration セクションを「決定 1-16 + 決定 1-3」→「決定 1-16 + 決定 1-3 + 決定 1-17」に拡張、R9.1 改版に 3 項目統合明示

requirements.md 改版: 本ラウンドで新規確定 (R9.1 「未使用カテゴリ INFO」追加、決定 1-17)、本 design approve 後の連鎖 migration で R9.1 改版 1 commit (決定 1-16 + 決定 1-3 + 決定 1-17 統合) に集約

- 2026-04-28: Spec 1 やり直しラウンド 6 (失敗モード handler + 観測性) — 新方式 4 重検査 + Step 1b-v 自動深掘り 5 切り口 + 厳しく検証 default + negative 視点強制発動 適用、**検出なし** (Phase 1 R7 escalate Eventual Consistency 規範化結果を維持、Error Categories 11 項目 / Transaction Guarantee / Monitoring / Out of scope 全件整合):
  - 5 番目 negative 視点強制発動の skim 防止証跡: 破綻シナリオ列挙試行 → 該当なし (stash 機構の Phase 3 委譲明示で運用継続性維持)、Phase 1 アナロジー 3 種比較 → いずれも非同型 (Spec 0 R4 escalate 規範範囲先取り = 該当なし / Spec 1 R5 escalate 文書 vs 実装不整合 = API 名整合済 / Spec 1 R7 escalate Eventual Consistency = 既採択維持)

requirements.md 改版: なし

- 2026-04-28: Spec 1 やり直しラウンド 7 (セキュリティ / プライバシー具体化) — 新方式 4 重検査 + Step 1b-v 自動深掘り 5 切り口 + 厳しく検証 default + negative 視点強制発動 適用、計 1 件適用 (要件改版なし、design 内修正のみ、**Spec 0 R1 重-厳-4 同型問題の再発検出**):
  - **重-7-1** (escalate 確証手順、design 内修正): Spec 1 Security Considerations に「**Subprocess shell injection 防止**」規律 1 段落を追加 (`shell=False` 必須、args list 形式、user 入力を直接 shell に渡さない、Phase 2 Spec 4 design 実装で確定)。Spec 0 design L853 と同等規律、Spec 0 R1 重-厳-4 (subprocess shell injection 防止規律未明示) と **同型問題再発検出** = TODO_NEXT_SESSION.md「Spec 1 やり直しでも同型問題を意識的に発掘する」要請への応答。Spec 1 Security Considerations と Spec 0 Security Considerations の構造均一化

requirements.md 改版: なし

- 2026-04-28: Spec 1 やり直しラウンド 8 (依存選定) — 新方式 4 重検査 + Step 1b-v 自動深掘り 5 切り口 + 厳しく検証 default + negative 視点強制発動 適用、計 1 件適用 (要件改版なし、design 内修正のみ、**Phase 1 R10 やり直し後の整合化漏れ検出**):
  - **重-8-1** (escalate 確証手順、design 内修正): Allowed Dependencies L68 を「Python 標準ライブラリ」+「第三者ライブラリ (PyPI)」に分離明記。`pyyaml` を「Python 標準ライブラリ」枠から「第三者ライブラリ (PyPI)」枠へ移動、`sqlite3` を Python 標準枠に追加 (決定 1-8 Phase 2 cache 用)。L142 Technology Stack table と整合化、Phase 1 R10 やり直しの change log L1327「pyyaml は第三者ライブラリ訂正 (Python 標準ではない)」記述あるが L68 への反映漏れを解消。Spec 0 R7-R10 重-厳-5 と同型問題 (単純誤記 grep + 整合化漏れ修正)

requirements.md 改版: なし

- 2026-04-28: Spec 1 やり直しラウンド 9 (テスト戦略) — 新方式 4 重検査 + Step 1b-v 自動深掘り 5 切り口 + 厳しく検証 default + negative 視点強制発動 適用、計 1 件適用 (要件改版なし、design 内修正のみ):
  - **軽-9-1** (自動採択、design 内修正): Integration Tests Transaction Guarantee Integration Tests の脚注「第 7 ラウンド」を「Phase 1 旧 R7 escalate 対応、新 10 ラウンド構成では R6 失敗+観測性に統合」に整合化 (memory 改訂版 12 → 10 ラウンド統合に対応)
  - 5 番目 negative 視点強制発動の skim 防止証跡: 破綻シナリオ列挙試行 → 該当なし (Cross-spec Integration Tests ハイブリッド方式で provider 側 test 重複記述リスクは memory 規律 + consumer 側委譲明示で解消)、Phase 1 アナロジー 3 種比較 → いずれも非同型
