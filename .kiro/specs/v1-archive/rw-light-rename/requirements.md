...
## Boundary Context
- **In scope**: `scripts/rw_light.py` → `scripts/rw_cli.py` のファイルリネーム、`cmd_init` の symlink 作成ターゲット更新、テスト側 ~500 件の `rw_light` 参照を `rw_cli` 参照に一括置換、`docs/` および `.kiro/steering/` の参照更新、`templates/` 配下のテンプレートファイル（`templates/CLAUDE.md` 等）と `tests/conftest.py::mock_templates` fixture の `scripts/rw_light.py` ダミーファイル生成 (cmd_init の os.stat 検査対応) が `rw_light.py` に言及する場合の参照更新、CI / 依存設定ファイル (`pyproject.toml` / `setup.py` / `setup.cfg` / `.pre-commit-config.yaml` / `Makefile` / `.github/workflows/*.yml`) の `rw_light` 参照確認と更新（事前調査で該当ファイルに参照なしを確認済みだが、本スペック適用時点で再確認し該当箇所があれば更新する）、既存 Vault 向けマイグレーションヘルパとしての `rw init --reinstall-symlink` サブフラグ追加、**Req 5 の `--reinstall-symlink` 機能に対する自動テストの新規追加**（pytest テストカバレッジ確保のため）。
- **Out of scope**: コマンド動作変更、Req 5 以外の新機能追加、既存テストの論理変更（monkeypatch パターンの変更・アサーション論理変更等、patch 対象および直接アクセス参照の文字列更新のみ許可）、他 5 モジュール (`rw_config`, `rw_utils`, `rw_prompt_engine`, `rw_audit`, `rw_query`) の内容変更、`rw_light.py` を bridge/proxy ファイルとして残す案（Req 1.3 に抵触するため不採用）。
- **Adjacent expectations**: module-split スペックの Req 6.1（「`scripts/rw_light.py` が存在し続ける」）は本スペック適用時点で失効し、`scripts/rw_cli.py` に読み替える契約に更新される。既存 spec 本文 (`.kiro/specs/module-split/requirements.md` Req 6.1 等) の `scripts/rw_light.py` 言及原文に対する編集 / 補注追加 / 不変のいずれを採るかは design フェーズの取り扱い事項とする（歴史性保持の原則は Req 6.3 で維持）。既存 Vault（`rw` symlink が旧 `rw_light.py` を指す状態）は現時点で実体なしと確認済みだが、将来ユーザ向けの再構成手順は `rw init --reinstall-symlink` で提供する。
...
#### Acceptance Criteria
1. When the full test suite is run after the rename (`pytest tests/`), the refactored codebase shall pass 現行件数以上（641 passed + 1 skipped = 642 collected 以上、リグレッションなし）。Req 5 の `--reinstall-symlink` 機能カバレッジ確保のため本スペックで新規テストを追加することは許容されるが、追加対象は `--reinstall-symlink` 関連（symlink 張り替え挙動・非 Vault validation・`--force` 併用警告）に限定し、既存テストの論理変更（monkeypatch パターン変更・アサーション論理変更等）は引き続き禁止する。
2. The test files shall contain zero import statements referencing the `rw_light` module in any form（`import rw_light` / `from rw_light import <symbol>` / `from scripts.rw_light import <symbol>` 等、全て `rw_cli` 参照に機械置換済み）。
3. The test files shall contain zero `monkeypatch.setattr` references to `rw_light` in either object form (`monkeypatch.setattr(rw_light, "<symbol>", ...)`) or string form (`monkeypatch.setattr("rw_light.<symbol>", ...)`), covering residual commands and helpers (`cmd_lint` / `cmd_ingest` / `cmd_synthesize_logs` / `cmd_approve` / `cmd_init` / `_backup_timestamp` / `plan_ingest_moves` / `execute_ingest_moves` / `load_lint_summary` / `parse_topics` / `render_candidate_note` / `candidate_note_path` / `candidate_files` / `approved_candidate_files` / `synthesis_target_path` / `merge_synthesis` / `promote_candidate` / `mark_candidate_promoted` / `update_index_synthesis` / `append_approval_log`)（全て `rw_cli` 参照に機械置換済み）。
...
3. Historical 参照（git commit メッセージ, 過去セッション記録, 完了済 spec 本文の `rw_light.py` 言及等）は変更対象外（歴史性を保持する）。
