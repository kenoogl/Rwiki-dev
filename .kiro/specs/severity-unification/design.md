...
- runtime drift 検出は **デフォルト継続**（警告 + INFO 降格）、strict mode は optional flag
...
**各 phase の不変条件**: phase 完了時点で `pytest tests/` が全件 green。phase 境界を跨ぐ 1 PR は禁止。phase 内での細分化（例: P3 を lint と lint_query で分割）は許容。
...
| 1.9 | drift 可視化 | `_normalize_severity_token` | stderr + optional strict mode | Severity Drift 3 層防御 |
...
| 7.10 | Drift 可視化挙動（default 継続 / strict abort） | `tests/test_audit.py::test_drift_visibility` | `_normalize_severity_token` の未知トークン → stderr + INFO 降格、strict mode → exit 1 | Severity Drift 3 層防御 |
...
- **top-level `schema_version` フィールド新設（F-3 対応）**: 下流 JSON consumer が payload の新旧 schema を field 存在推測ではなく明示的に判別できるよう、文字列値 `"2.0"` を付与。新 schema（severity-unification 後）は `"2.0"`、旧 schema は `"1.0"` 扱い（存在しなければ `"1.0"` と見なす forward-compat rule）。**P1 transitional 期にも `"1.0"` を付与する forward-compatibility window を設ける**: P1 commit で `schema_version: "1.0"` を付与 → P3 commit で `"2.0"` に更新（`severity_counts` / top-level `status` 追加と同時）。これにより下流 consumer は rollback 時の schema 自動判定が可能になる
...
| 兆候 | 検知方法 | 対応手順 |
|---|---|---|
| drift 頻度の高騰 | `logs/audit-*-latest.md` の「Severity Drift Events」節が頻繁に出現、stderr end summary の `TOTAL: <N>` で N ≥ 10 が継続 | Prompt prefix 見直し、`--strict-severity` 一時適用で Claude 応答品質を原因特定、必要なら `build_audit_prompt` の prefix 文言を強化 |
...
| JSON consumer 破損 | 下流 dashboard / metrics 集計が null / KeyError / TypeError | `schema_version` 確認 → developer-guide.md Migration Notes §JSON Consumer Migration（E-3 対応表）に従い consumer を更新 |
...
| drift_events cap reached の常態化 | `drift_events[-1]` に sentinel が毎回出現、N > 100 が続く | adversarial Claude 応答または prompt 設計の bug を疑う、strict mode 強制で根本原因特定 |
...
**Post-merge チェック**:
- `rw audit weekly --validate-vault-only` を smoke test として実行
- `rw audit weekly` を通常実行し exit code を確認
- Acceptance Smoke Test 5-minute QA（I-5）の該当 phase 部分を実行

**将来 CI 統合時の recommended gate**（本スペック scope 外、将来の別 spec で扱う）:
- GitHub Actions で `pytest tests/` を macOS / Linux matrix 実行
- `pytest-repeat` で subprocess test の flakiness 検出（K-4）
- schema_version / Pattern A/B/C / test_docs_consistency.py の static verification を blocking gate
- coverage.py による line coverage ≥ 90% / branch coverage ≥ 85% の enforcement（K-3）
