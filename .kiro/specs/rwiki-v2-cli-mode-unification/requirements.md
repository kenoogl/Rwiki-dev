...
- **Out of scope**:
  ...
  - Skill 選択 dispatch ロジック（Spec 3 / 明示 → `type:` → category default → `applicable_input_paths` glob match → LLM 判断の 5 段階優先順位、段階 3.5 は Spec 2 Requirement 3.2 由来 coordination で 2026-04-27 確定）
...
3. If `--auto` 禁止リストに該当するコマンドが `--auto` 付きで実行された, then the rw CLI shall 即座に exit code 1 で失敗し stderr に「このコマンドは --auto を許可しません」と理由を出力する。禁止リストは `rw retract` / `rw query promote` / `rw split`（wiki page 分割は不可逆の構造変更のため禁止）/ `rw tag split` / `rw skill retract` の **5 種**とする（drafts §7.2 Spec 7 表 L2183-2197 と整合、Spec 7 Requirement 6.4 と一致）。**注記**: Spec 6 Requirement 9 が定める **`rw approve <hypothesis-id>`**（Hypothesis 専用昇格 CLI、id 指定）は本 spec の禁止リスト 5 種には未列挙だが、Spec 6 R9.8 / R9.9 で `--auto` 不可指定済（内部で Spec 7 `cmd_promote_to_synthesis` 8 段階対話 handler を呼ぶため `promote-to-synthesis` 内部 handler 視点での `--auto` 不可と整合）。本 spec の review 層 dispatch `rw approve [<path>]`（Requirement 16、path 指定で 6 review 層を判別）とは別 operation として扱う（Spec 6 R9.9 三者命名関係明示と整合）。The rw CLI shall 環境変数（例: `RWIKI_FORCE_AUTO=1`）や非標準起動オプションを介して **禁止リストの `--auto` を強制 on にするバイパス経路を提供しない**（バイパスを試みた場合も同様に exit code 1 で拒否する）。Maintenance UX（Requirement 7 / 8 の包括承認）も禁止リストに対する `--auto` バイパスとして機能してはならない（Requirement 7.5 と整合）。
...
4. The rw CLI shall `rw distill` / `rw query *` / `rw audit semantic` / `rw audit strategic` / `rw retag` の **CLI dispatch と Hybrid 実行契約** のみを所管し、Skill 選択 dispatch（明示 → `type:` → category default → `applicable_input_paths` glob match → LLM 判断の 5 段階優先順位、Spec 3 Requirement 1、段階 3.5 は Spec 2 Requirement 3.2 由来 coordination で 2026-04-27 確定）を Spec 3 の所管として、Skill ファイルのスキーマと `.rwiki/skills/` 構造を Spec 2 の所管として明示する。
...
1. When ユーザーが `rw approve [<path>]` を実行した, the rw approve command shall `<path>` から該当 review 層を判別し（または `<path>` 省略時は全 review 層を順次 scan し）、以下の 6 review 層に対応する dispatch 先を決定する: (a) `review/synthesis_candidates/*` / (b) `review/vocabulary_candidates/*` / (c) `review/audit_candidates/*` / (d) `review/relation_candidates/*` / (e) `review/decision-views/*` / (f) `wiki/.follow-ups/*`（Follow-up）。**注記**: `review/hypothesis_candidates/*` は本 spec の path 指定 dispatch の **対象外**。Hypothesis 昇格は Spec 6 Requirement 9 が定める **`rw approve <hypothesis-id>`**（id 指定 CLI）経由で Spec 7 `cmd_promote_to_synthesis` 8 段階対話 handler を呼ぶ独立 operation（Spec 6 R9 / R9.9 と整合、本 spec の `rw approve [<path>]` review 層 dispatch とは別系統）。`<path>` が `review/hypothesis_candidates/*` を指定された場合は stderr に「Hypothesis 昇格は `rw approve <hypothesis-id>` を使用してください（Spec 6 R9）」と通知し exit code 1 で終了する。
...
- 2026-04-27 (Adjacent Sync): Spec 3 第 5 ラウンド由来 — Skill 選択 dispatch 優先順位記述を 4 段階 → 5 段階化、2 箇所同期（Boundary Context Out of scope line 51 + R13.4 line 254）。新段階 3.5（`applicable_input_paths` glob match）は Spec 2 Requirement 3.2 由来 coordination で 2026-04-27 確定、本 spec は dispatch 所管 spec が Spec 3 である事実のみ規定する立場を維持。Adjacent Spec Synchronization 運用ルールに従い再 approval は不要、`spec.json.updated_at` 更新のみ。
- 2026-04-27 (Adjacent Sync): Spec 6 第 1 ラウンド 重-3 (A-2) + 第 5 ラウンド 第 5-C 由来 — (D-27) Requirement 3.3 禁止リスト 5 種に **Spec 6 R9 `rw approve <hypothesis-id>`（Hypothesis 専用昇格 CLI、id 指定）** が未列挙であることに対する注記追加（Spec 6 R9.8 / R9.9 で `--auto` 不可指定済、内部 Spec 7 `cmd_promote_to_synthesis` 経由、本 spec の review 層 dispatch `rw approve [<path>]` とは別 operation として扱う三者命名関係を明示）。(D-28) Requirement 16.1 review 層 6 種 dispatch 規定に `review/hypothesis_candidates/*` の扱いを注記追加（path 指定 dispatch の対象外、id 指定 `rw approve <hypothesis-id>` 経由で Spec 6 R9 が所管、path 指定された場合は stderr 通知 + exit code 1）。Adjacent Spec Synchronization 運用ルールに従い再 approval は不要、`spec.json.updated_at` 更新のみ。
