...
- **Out of scope**:
  ...
  - **Skill 選択 dispatch**: Spec 3（明示 → `type:` → category default → `applicable_input_paths` glob match → LLM 判断の 5 段階優先順位、段階 3.5 は Spec 2 Requirement 3.2 由来 coordination で 2026-04-27 確定）。
...
2. **[P0]** The Decision Log shall `decision_type` の **初期セット** を以下 23 種として規定し、起源 spec ごとに区分することを明示する。
   ...
   - **Page lifecycle 起源（7 種、Spec 7 所管、Spec 7 Requirement 12.7 と整合、順序は Spec 7 Requirement 3.8 の `update_history.type` と整合）**: `page_deprecate` / `page_retract` / `page_archive` / `page_merge` / `page_split` / `page_reactivate` / `page_promote_to_synthesis`
...
6. **[P0]** The Decision Log shall reasoning 必須（skip 不可）対象を以下 4 条件として固定することを規定する（`reasoning_input.require_for`）: `hypothesis_verify_confirmed` / `hypothesis_verify_refuted` / `synthesis_approve` / `page_retract`。
...
15. **[P1]** The Decision Log shall **Decision Log 健全性診断 API** として `check_decision_log_integrity() → IntegrityReport` を提供し、以下 4 診断項目を計算可能にすることを規定する（Spec 4 Requirement 4.5 / 8.2 と整合、`rw doctor` および Maintenance UX autonomous surface から呼び出される）。
    ...
    - **schema 違反件数**: 必須 10 field（Requirement 11.1）の欠落 / `decision_type` 23 種（Requirement 11.2）外の不正値 / `actor` / `subject_refs` 等の型違反
...
16. **[P0]** The Decision Log shall **partial failure 記録形式** として、`outcome` field 内に `partial_failure: bool` / `successful_edge_ops: int` / `failed_edge_ops: int` / `followup_ids: List[str]` の 4 field をサポートし、Spec 7 Requirement 3.7 / 3.8 から渡される partial failure 情報を `decision_type: page_*`（Page lifecycle 起源 7 種）の decision として記録可能にすることを規定する（Spec 7 Requirement 12.8 由来の edge API timeout も partial failure として `failed_edge_ops` に計上、Requirement 18.7 / 18.8 と整合）。
...
6. **[P0]** The L2 Graph Ledger shall `decision_log.*` セクションに以下の項目を所管することを規定する。
   ...
   - `reasoning_input.require_for`（list: `hypothesis_verify_confirmed` / `hypothesis_verify_refuted` / `synthesis_approve` / `page_retract`）
...
- 2026-04-27 (Adjacent Sync): Spec 3 第 5 ラウンド由来 — Boundary Context Out of scope の「Skill 選択 dispatch」記述を 4 段階 → 5 段階化（明示 → `type:` → category default → `applicable_input_paths` glob match → LLM 判断）。新段階 3.5（`applicable_input_paths` glob match）は Spec 2 Requirement 3.2 由来 coordination で 2026-04-27 確定、本 spec は dispatch 所管 spec が Spec 3 である事実のみ規定する立場を維持。Adjacent Spec Synchronization 運用ルールに従い再 approval は不要、`spec.json.updated_at` 更新のみ。
- 2026-04-28 (Adjacent Sync): Spec 7 design Round 1 重-1-2 + Round 3 重-3-4 由来の連動更新 2 件を反映 — (1) **R11.6 の reasoning 必須 4 種列挙のうち `'retract'` を `'page_retract'` に改版**（Spec 7 R12.7 命名規則整合、本 spec L288 + Configuration L507 同時更新）。(2) **R11.2 を `decision_type` 22 種 → 23 種に拡張**、Page lifecycle 起源を「6 種」→「7 種」に再分類して `page_reactivate` を追加（Spec 7 R12.7 拡張と同期、Spec 7 design L551 cmd_reactivate handler が `decision_type=page_reactivate` で記録する旨と整合、`page_reactivate` の挿入位置 = `page_split` と `page_promote_to_synthesis` の間 = Spec 7 R3.8 update_history.type 列挙順序整合）。連鎖更新 4 箇所: R11.2 文言（line 266 / 271）/ R11.15 schema 違反件数の「22 種」記述（line 301）/ R11.16 partial failure 記録対象「Page lifecycle 起源 6 種」（line 303）/ Phase 表 P0 概要文の「22 種 decision_type」（line 564）を 23 種 / 7 種に統一。本 spec requirements 改版のため再 approval 必要。
