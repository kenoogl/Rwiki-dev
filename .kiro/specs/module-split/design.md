...
- `rw_light.call_claude` 以外のシンボルの後方互換 re-export（AC 1.3 が明示するのは `call_claude` のみ。他シンボルへの運用スクリプト依存が発覚した時点で個別対応する。網羅 re-export は構造的負債となるため採用しない）。

> **Re-export 戦略の確定（review-driven update）**: 初稿では `call_claude` のみ re-export とし、レビューで判明した約 299 件のテスト直接アクセス（test_rw_light.py 208, test_utils.py 28, test_audit.py 23, test_lint_query.py 22, test_git_ops.py 7, conftest.py 5, test_init.py 2, test_lint.py 2, test_conftest_fixtures.py 2）に対しては、一時、網羅 re-export 案（Option A）が検討された。しかし fundamental review で以下が指摘された:
...
>
> よって最終方針: **テスト本体の `rw_light.<symbol>` 直接アクセス 299 件を `rw_<module>.<symbol>` 形式に機械置換し、re-export は AC 1.3 が明示する `call_claude` のみに限定する**。一時コスト（299 件の sed ベース置換 ≈ 数時間〜半日）を支払い、永続負債（モジュール境界の不可視化、メンタルモデル不整合）を回避する。詳細は「Components and Interfaces > rw_light」および Migration Strategy の各 Phase X.2 タスク参照。
...
# 後方互換 re-export（AC 1.3 — call_claude のみ）
# 注意: 外部運用スクリプト専用。テストは rw_<module>.<symbol> 形式で patch かつ access すること。
...
- Re-export 限定方針: AC 1.3 が明示する `call_claude` のみを re-export。テスト本体は各 Phase X.2 で `rw_light.<symbol>` 直接アクセスを `rw_<module>.<symbol>` 形式に機械置換し、patch 先と access 先を対称化する（モジュール境界の可視化）。
...
- Risks: AC 1.3 が明示する `call_claude` 以外への外部運用スクリプト依存が後日発覚した場合、個別 re-export 追加で対応。網羅 re-export には戻さない（fundamental review で構造的負債と判定済み）。テスト書き換えは sed/正規表現で機械化可能だが、`rw_light.X` (no parens) や `from rw_light import X` 形式の検出漏れを防ぐため、Phase X.2 では複数パターンで grep verify する。
...
| `tests/test_conftest_fixtures.py` | 2 | rw_config / rw_utils |
...
flowchart TB
    ...
    Phase4b --> Phase5[Phase 5: rw_light スリム化<br/>+ call_claude re-export 確認<br/>+ main dispatch 最終整理]
...
| 3 | `rw_prompt_engine.py` 作成、`call_claude` / `load_task_prompts` / `read_wiki_content` / `read_all_wiki_content` / `parse_agent_mapping` / `build_query_prompt` / `call_claude_for_log_synthesis` / `_validate_agents_severity_vocabulary` を移動。**併せて `rw_light.py` に AC 1.3 互換 re-export `from rw_prompt_engine import call_claude` を追加**（外部運用スクリプト互換のみ。他 prompt engine シンボルは re-export しない） | `test_rw_light.py` / `test_audit.py` / `test_synthesize_logs.py` の該当 **monkeypatch 先**を `rw_prompt_engine` 参照に更新。**加えて `rw_light.<prompt_engine_func>` 形式の直接アクセス（test_audit.py の severity 関連 + load_task_prompts / read_wiki_content / read_all_wiki_content 等、test_rw_light.py 該当分）を `rw_prompt_engine.<func>` に書き換え、必要なテストファイルに `import rw_prompt_engine` を追加。ただし `rw_light.call_claude` 直接アクセスは re-export 維持のため書き換え不要（任意で書き換え）** | `pytest tests/` green |
