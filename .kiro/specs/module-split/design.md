...
- `rw_light.call_claude` 以外のシンボルの後方互換 re-export（Req 1.3 が明示するのは `call_claude` のみ。他シンボル（`today`, `git_commit` 等）に運用スクリプト依存が発覚した時点で個別対応する。本スペックでは投機的な網羅 re-export をしない）。
...
- `rw_light.call_claude` 等の後方互換アクセスを `rw_light.py` 内の再 import で維持する設計。
...
└── rw_light.py           # 残存コマンド + main() + ディスパッチ + 後方互換 re-export
```
...
- **`scripts/rw_light.py`** — 大幅縮小（~3,827 行 → ~700 行見積）。
  ...
  - 追加: 後方互換 re-export（`from rw_prompt_engine import call_claude` — Req 1.3 対応）
...
**test_lint.py, test_git_ops.py, test_utils.py, test_init.py, test_lint_query.py, test_agents_vocabulary.py, test_source_vocabulary.py, test_conftest_fixtures.py** については `monkeypatch.setattr(rw_light, ...)` の直接参照が無い／あるいは `conftest.py` のフィクスチャ経由でのみ関与するため、本体ファイル自体の修正は不要または軽微（`conftest.py` 更新で自動追随）。
...
| 1.3 | `rw_light.call_claude` 継続アクセス | `rw_light.py` 再 export | `from rw_prompt_engine import call_claude` | — |
...
| rw_light | 4 (Dispatch) | ディスパッチャ + 残存コマンド + 後方互換 | 1.1, 1.3, 6.1, 6.2, 6.3 | 上記 5 モジュール (すべて P0) | Service |
...
| Field | Detail |
|-------|--------|
| Intent | エントリポイント。残存コマンド実装とサブモジュールへのディスパッチ、および後方互換 re-export。 |
...
- 後方互換: `from rw_prompt_engine import call_claude` 等の re-export（Req 1.3）。
...
# 後方互換 re-export（Req 1.3）
# 注意: 外部運用スクリプト専用。テストは rw_prompt_engine.call_claude を patch すること（Req 4.1）。
from rw_prompt_engine import call_claude  # noqa: F401
# 他の外部公開シンボルは必要が発覚次第追加
```
...
- Risks: 後方互換 re-export の対象を絞り込みすぎると外部運用スクリプトが壊れる。逆に過剰に re-export すると分割の意味が薄れる。初期は `call_claude` のみ（Req 1.3 が唯一明示する対象）から開始し、問題発覚時に追加。
...
flowchart TB
    ...
    Phase4b --> Phase5[Phase 5: rw_light スリム化<br/>+ 後方互換 re-export 追加<br/>+ main dispatch 更新]
...
|-------|------------|------------|----------|
| 1 | `rw_config.py` 作成、全定数移動、`rw_light.py` を `import rw_config` + `rw_config.X` 参照に書き換え | `tests/conftest.py` の `patch_constants` fixture を `rw_config` 参照に更新 + `test_rw_light.py` / `test_audit.py` 内の直接定数パッチを `rw_config` 参照に一括置換 | `pytest tests/` green |
| 2 | `rw_utils.py` 作成、ユーティリティ関数群を移動、`rw_light.py` を `rw_utils.X` 修飾参照に書き換え | `test_approve.py` の `git_path_is_dirty` / `warn_if_dirty_paths`、`test_ingest.py` の `git_commit`、`test_synthesize_logs.py` の `git_path_is_dirty`、`test_rw_light.py` の `today` / `_git_list_files` 等を `rw_utils` 参照に更新 | `pytest tests/` green |
| 3 | `rw_prompt_engine.py` 作成、`call_claude` / `load_task_prompts` / `read_wiki_content` / `read_all_wiki_content` / `parse_agent_mapping` / `build_query_prompt` / `call_claude_for_log_synthesis` / `_validate_agents_severity_vocabulary` を移動。**同時に `rw_light.py` に後方互換 re-export `from rw_prompt_engine import call_claude` を追加**（Req 1.3 の中間状態違反を防止） | `test_rw_light.py` / `test_audit.py` / `test_synthesize_logs.py` の該当パッチを `rw_prompt_engine` 参照に更新。`_validate_agents_severity_vocabulary` 関連のテスト参照も `rw_prompt_engine` に更新 | `pytest tests/` green |
| 4a | `rw_audit.py` 作成、audit コマンドとチェック関数群（`Finding` / `WikiPage` NamedTuple + `_normalize_severity_token` / `_record_drift` / `_resolve_link` 含む）を移動、`rw_light.main()` ディスパッチを `rw_audit.cmd_audit` 呼び出しに切り替え | `test_audit.py` / `test_rw_light.py` の audit 系関数パッチ（`_normalize_severity_token` / `_record_drift` 含む）を `rw_audit` 参照に更新 | `pytest tests/` green + **`wc -l scripts/rw_audit.py` で 1,500 行以内を確認** |
| 4b | `rw_query.py` 作成、query コマンドと query lint 関数群（`_strip_code_block` 含む）を移動、`rw_light.main()` ディスパッチを `rw_query.cmd_query_*` / `rw_query.cmd_lint_query` 呼び出しに切り替え | `test_rw_light.py` の `lint_single_query_dir` および `cmd_query_extract/answer/fix` パッチを `rw_query` 参照に更新 | `pytest tests/` green |
| 5 | `rw_light.py` 最終スリム化（残存コマンドのみに）、`main()` + `print_usage()` 最終化。後方互換 re-export は Phase 3 で既に追加済み | 残存するテスト参照の最終確認・整合性チェック | `pytest tests/` で 642 件以上 green、`python scripts/rw_light.py` で usage 表示 |
