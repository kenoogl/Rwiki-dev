...
> 出典: `.kiro/drafts/dual-reviewer-draft.md` v0.3 §3.1 A-2 / §3.5 / §2.10
...
- ドラフト v0.3 §3.1 A-2 + §3.5 で対照実験設計確定 (全 Round で single + dual の両方完走、cost 倍)
- ドラフト v0.3 §2.10.3 で論文 figure 候補確定 (figure 1-5、本 spec で取得するのは figure 1-3 = quantitative)
...
- Spec 6 design Round 1-10 完走 + dual-reviewer 適用 review evidence 提供 = Spec 6 spec 経由で Rwiki v2 design phase 全 8 spec approve 達成可能化 (approve 操作自体は Spec 6 spec の責務、本 spec は enablement 立場)
...
- single 系統と dual 系統で `miss_type` / `difference_type` / `trigger_state` を同 schema で記録 (single でも自己ラベリング、dual との差を absent / present の binary 識別で可視化、`dual-reviewer-design-review` Req 6 AC 6.6 整合)
...
- single 系統でも `miss_type` / `trigger_state` を自己ラベリング (`difference_type` は dual mode のみ = single mode で absent、design-review Req 6 AC 6.6 整合、Approach L36 absent / present binary 識別と整合)。single の trigger failure 率と dual の比較で adversarial 効果を定量化、単純な「dual で多く拾えた」を超えた構造的 evidence
