...
6. While 3 系統対照実験が cost 約 3 倍 (= V3 baseline = 1 系統 dual に対し本 spec = 3 系統) を要求する間, the dogfeeding session shall 以下の constraint を採用する: (a) **判定 7-C** (= cost 3 倍許容、論文 8 月ドラフト提出 timeline 厳守、ドラフト v0.3 §3.1 A-2 + §3.5 で確定済設計判断、本 AC で inline 展開 = SSoT chain 1-hop 限定) を採用 / (b) 論文 8 月ドラフト提出 timeline failure 基準として **8 月末日までに figure 1-3 用データ生成 (Req 5 AC1-AC3 達成) + V4 ablation evidence (Req 5 AC4 達成) が完了していない場合を timeline 未達** と定義し、未達時は Phase B fork hold 判定の補助根拠として comparison-report (Req 6 AC4) に明記する.
...
2. The figure data generation shall, **figure 2 用 data** = `difference_type` 6 enum 分布 (= `assumption_shift` / `perspective_divergence` / `constraint_activation` / `scope_expansion` / `adversarial_trigger` / `reasoning_depth` の件数 + 比率) + forced_divergence 効果 (= dual / dual+judgment 系統で `difference_type = adversarial_trigger` の発動件数 + 比率) を算出し、**dual vs single で adversarial 効果分離** を可能とする (brief.md Desired Outcome 整合); ここで forced_divergence 効果計測は **design-review spec design phase 完走** (= forced_divergence prompt template 文言確定 = design-review spec Req 6 AC1 達成後) を前提として実施する (= sequencing 制約 = design-review design approve 前の dogfeeding 適用は本 spec scope 外).
...
3. The dogfeeding session shall, **Spec 6 design approve** (= `.kiro/specs/rwiki-v2-perspective-generation/spec.json` `approvals.design.approved: true` 状態、= Spec 6 spec 自身の責務として完了する事項) を **本 spec 完了の前提条件 (= 確認条件)** として扱い、Spec 6 design approve 成立後に本 spec 完了を宣言する; これにより 本 spec dogfeeding 完了 = Spec 6 design approve 成立 = **Rwiki v2 design phase 全 8 spec approve 完了** = **Phase A 終端** が成立する (= 本 spec が他 spec の approve 状態を強制する責務はなく、Spec 6 spec 側の approve 完了を確認条件として扱う、= 責務境界整合).
