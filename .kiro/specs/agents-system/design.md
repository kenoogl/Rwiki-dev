...
**Purpose**: CLAUDE.mdカーネルの肥大化を防ぎつつ、Claudeがタスク実行時に適切なルールをオンデマンドで読み込めるAGENTS/サブプロンプト体系を構築する。仕様案の9タスク種別に対応する9エージェントファイル、3ポリシー、およびカーネル更新を `templates/` に配置する。
...
- 3ポリシー（page_policy, naming, git_ops）を独立ファイルとして配置
...
| ポリシー | Policy | タスク横断的なルールを独立管理 | 4.1-4.6 | docs/ 素案 |
...
3. **エージェントロード**: 特定されたエージェントファイルとポリシーを読み込む
...
2. マッピング表のポリシー列に記載されたポリシーを Read ツールで読み込む
...
ポリシーはエージェントテンプレートとは異なり、簡潔なルール定義形式:
...
- ポリシーは docs/ の素案をほぼそのまま使用可能（page_policy: 30行、naming: 47行、git_ops: 65行）。共通テンプレートのフォーマットに軽微な調整のみ必要
...
- query_extract（query_extract.md）の出力契約は cli-query スペックの前提条件となる。docs/query.md に定義された出力構造（`review/query/<query_id>/` ディレクトリ内に question.md, answer.md, evidence.md, metadata.json の4ファイル）を精査時に保持すること。この契約はエージェントファイルの Output セクションに明記する
...
| ポリシー存在 | templates/AGENTS/ | 3ファイル存在確認 | 4.1 |
