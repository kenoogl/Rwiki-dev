...
9. The Spec 6 Perspective-Hypothesis Subsystem shall **三者命名関係** を以下のとおり明示することを規定する（drafts SSoT 整合）。
   ...
   - Spec 4 Requirement 3.3 / 3.5 禁止リストには本 spec 主名称 `rw approve <hypothesis-id>` が **未列挙**（5 種固定）であるが、本 AC 8 で `--auto` 不可と確定する。Spec 4 への Adjacent Sync で R3.3 注記追加（「`rw approve <hypothesis-id>` は Spec 6 Requirement 9 で `--auto` 不可指定済、本 spec の review 層 dispatch `rw approve [<path>]` とは別 operation」）を要請する。**(2026-04-27 Adjacent Sync 適用済)**: 本要請に基づき Spec 4 R3.3 line 109 (D-27) および Spec 4 R16.1 line 298 (D-28) に注記追加完了、`review/hypothesis_candidates/*` を path 指定 dispatch 対象外として明示済（Spec 4 spec.json.updated_at = 2026-04-27T16:50:37+09:00、Adjacent Spec Synchronization 運用ルール準拠で再 approval 不要）。
...
- 2026-04-27 (cross-spec 全 8 spec 整合性精査由来 軽-A 反映、(β) 案): R9.9 末尾に Spec 4 D-27 + D-28 Adjacent Sync 適用済の現状反映を追記 — 「Spec 4 R3.3 line 109 (D-27) および R16.1 line 298 (D-28) に注記追加完了、Spec 4 spec.json.updated_at = 2026-04-27T16:50:37+09:00、Adjacent Spec Synchronization 運用ルール準拠で再 approval 不要」を明示。本セッション中の Adjacent Sync 完了状態を文書化レベルで Spec 6 自身に逆参照として記録 (Adjacent Spec Synchronization 運用ルール準拠、Spec 6 自身は再 approval 不要、`spec.json.updated_at` 更新のみ)。
