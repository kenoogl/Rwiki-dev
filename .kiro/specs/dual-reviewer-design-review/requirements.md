...
- **Adjacent expectations** (隣接 spec / 既存 system からの期待):
  ...
  - `dual-reviewer-dogfeeding` は本 spec の 3 skills (`dr-design` / `dr-log` / `dr-judgment`) + 本 spec design phase で確定する forced_divergence prompt template (Req 6 整合、英語固定) を Spec 6 design に適用 + 3 系統対照実験 (single + dual + dual+judgment) で運用する前提、本 spec は 3 skills + forced_divergence prompt template の動作可能性確認 (sample 1 round 通過) のみを責務とする
...
10. The dr-design skill shall, `--treatment` flag (`single` | `dual` | `dual+judgment`) を起動 args として受領し、treatment 値に応じて Step B (adversarial review) + Step C (judgment) の起動制御を切替える: (a) `single`: Step B / Step C 全 skip + Step D は primary 検出のみ user 提示 / (b) `dual`: Step B 実行 + Step C skip + Step D は primary + adversarial 検出 + adversarial counter_evidence を user 提示 (judgment subagent 不在のため V4 §2.5 三ラベル自動分類なし、全 finding を `should_fix` + `recommended_action: user_decision` として user 個別判断 prompt) / (c) `dual+judgment`: Step B / Step C 全実行 (V4 完全) + Step D は judgment 出力の三ラベル分類で user 提示; treatment 値は dr-log skill `open()` payload にも transparent に渡され、JSONL の各 review_case line に必須付与される (`dual-reviewer-dogfeeding` spec Req 3.6 整合).
...
4. The dr-log skill shall, JSONL 形式で append-only に dev-log JSONL location 配下に記録し、1 review session = 1 line + 1 review_case object を厳守する; 具体 JSONL append target path は foundation `dr-init` skill が生成する `config.yaml` の `dev_log_path` field (foundation Req 2 AC2 整合 = default `.dual-reviewer/dev_log.jsonl`) から動的に読み取る (hardcode 禁止).
...
8. The dr-log skill shall, `open()` 起動時に `timestamp_start` (ISO8601 UTC) を current time から自動 capture して in-memory session state に保持し、`flush()` 起動時に `timestamp_end` (ISO8601 UTC) を current time から自動 capture し、両 field を 1 review_case JSONL line に必須付与する; これにより 3 系統対照実験で Round wall-clock を計測可能とする (`dual-reviewer-dogfeeding` spec Req 3.6 整合).
9. The dr-log skill shall, `open()` payload で `design_md_commit_hash` を必須受領し (= dr-design skill が Round 起動時 `git rev-parse` で取得して渡す)、session state に保持して `flush()` 時の review_case object に必須付与する; これにより JSONL 各 entry で reproducibility (target design.md の特定 git commit hash 紐付け) を保証する (`dual-reviewer-dogfeeding` spec Req 3.7 整合).
...
- **v1.0** (2026-04-30 6th セッション、req phase approve 時): 7 Requirement (= 7 Req × 5-9 AC) で初版確定、req phase V3 / V4 review 適用後 approve。
- **v1.2** (2026-05-01 15th セッション、A-1 implementation phase 直前 cycle、cross-spec review C1 fix の req phase 整合化): design.md / tasks.md には既反映済の 3 改修要件を req phase AC として明示化 = (a) Req 1 AC10 追加 = dr-design `--treatment` flag (single | dual | dual+judgment) で Step B/C 起動制御切替 + Step D user 提示三ラベル制御 + treatment 値 dr-log payload 透過 / (b) Req 2 AC8 追加 = dr-log `open()` 時 `timestamp_start` 自動 capture + `flush()` 時 `timestamp_end` 自動 capture + 両 field review_case JSONL line 必須付与 / (c) Req 2 AC9 追加 = dr-log `open()` payload で `design_md_commit_hash` 必須受領 + session state 保持 + flush() 時 review_case 必須付与。design.md v1.2 + tasks.md v1.1 と整合済 (dogfeeding spec Req 3.6 / 3.7 整合)。req phase は実質的内容変更なし (= 既存設計 + 既存タスクの AC 整合化のみ)、approve status 維持。
