...
- **Out of scope**: frontmatter フィールドの詳細スキーマと vocabulary 実装（Spec 1 が所有）/ L2 Graph Ledger の data model と API の実装（Spec 5 が所有）/ 個別 CLI コマンドの実装（Spec 4 が所有）/ Skill 設計と dispatch の実装（Spec 2 / Spec 3 が所有）/ Page lifecycle 操作の実装（Spec 7 が所有）/ Perspective / Hypothesis 生成ロジックの実装（Spec 6 が所有）/ L1 raw および L3 wiki の subdirectory 規約と内部構造（`raw/incoming/` / `raw/llm_logs/` / `wiki/concepts/` / `wiki/methods/` / `wiki/synthesis/` 等のレイアウトは Spec 1 と steering `structure.md` が SSoT）/ 規範文書の内容妥当性の機械検証（人間レビュー gate に委譲、design phase での verifier 構築は brief.md の testability スコープに従い限定的）。
...
**Objective:** As a Spec 1-7 起票者および将来の v2 開発者・実装者・保守者, I want Rwiki v2 のビジョンと中核価値を SSoT として参照できる Foundation, so that 各 spec が独自解釈ではなく共通の価値前提に基づいて設計される。
...
**Objective:** As a Spec 1-7 起票者および将来の v2 開発者・実装者・保守者, I want L1/L2/L3 各層の責務・物理格納位置・更新頻度・人間関与の差を一意に定義した Foundation, so that 各 spec が層境界を誤らず、層をまたぐ責務配分を正しく行える。
...
3. The Foundation shall L2 Graph Ledger の構成ファイルとして `entities.yaml` / `edges.jsonl` / `edge_events.jsonl` / `evidence.jsonl` / `rejected_edges.jsonl` / `decision_log.jsonl` / `reject_queue/` を列挙し、いずれも JSONL/YAML の append-only または ledger 本体であること、derived cache（`.rwiki/cache/graph.sqlite`、`networkx.pkl`）は gitignore 対象であることを記述する。
...
**Objective:** As a Spec 1-7 起票者および将来の v2 開発者・実装者・保守者, I want 13 中核原則（§2.1-§2.13）の各々と、それらが L1/L2/L3 のどの層にどの強度で適用されるかを 1 つのマトリクスとして固定した Foundation, so that 各 spec が「全層一律適用」の誤解を避け、層ごとに異なる人間と LLM の関与モデルを反映できる。
...
7. The Foundation shall 次の **層別 binding を不変項目** として固定する: §2.10 Evidence chain は L1/L2/L3 全層を貫通する唯一の不変原則 / §2.12 Evidence-backed Candidate Graph は L2 のみに適用される / §2.2 Review layer first および §2.4 Dangerous ops 8 段階対話は L3 のみに必須 / §2.13 Curation Provenance は L2/L3 操作で機能し L1 では lint pass のみ。これらの bindings は design phase での再解釈・緩和を不可とする。
8. If 13 中核原則の追加・削除・改名・層別適用度の変更が提案された場合, then the Foundation shall その変更が本 Foundation requirements の改版（新 criterion 追加または既存 criterion 修正）と §2.0 マトリクスの同時更新を要し、roadmap.md の Adjacent Spec Synchronization 運用ルールでは扱わない（実質変更のため Spec 1-7 全体の再 approval 検討対象となる）ことを示す。

### Requirement 4: §2.12 Evidence-backed Candidate Graph 原則の優先関係

**Objective:** As a Spec 5 / Spec 6 起票者および将来の v2 開発者・実装者・保守者, I want §2.12 が L2 専用であり、§2.2 / §2.4 より優先するという関係性を明文で固定した Foundation, so that L2 Graph Ledger の設計が「全件 approve」「8 段階対話」を誤って要求しない。
...
**Objective:** As a Spec 1 / Spec 5 / Spec 7 起票者および将来の v2 開発者・実装者・保守者, I want Edge status と Page status を別次元として定義し、混同を防ぐ Foundation, so that frontmatter / ledger / lifecycle 設計で誤って共通の状態セットを使うことを防ぐ。
...
**Objective:** As a Spec 4 / Spec 6 起票者および将来の v2 開発者・実装者・保守者, I want コマンド階層 4 Level の分類とエディタとの責務分離ポリシーを Foundation で固定する, so that CLI 設計とユーザー UX 設計が「ユーザーは知識発見、メンテナンスは LLM ガイド」という上位原則を守る。
...
2. The Foundation shall L1 発見コマンドの代表として `rw chat` / `rw perspective` / `rw hypothesize` / `rw discover` を例示し、`rw discover` が R9.1 の discover task に対応する L1 発見コマンドであることを明示する。
...
**Objective:** As a Spec 1-7 起票者および将来の v2 開発者・実装者・保守者, I want 用語が 5 分類で整理された Foundation 用語集, so that 用語不統一による spec 間の解釈ずれを防ぐ。
...
4. The Foundation shall Graph Ledger 用語として Entity / Edge / Edge status / Page status / Confidence / Evidence ledger / Edge event / Dangling edge / Extraction mode / Graph Hygiene（Decay/Reinforcement/Competition/Contradiction tracking/Edge Merging）/ Usage signal / Reject queue / Curation provenance / Decision log / Decision type / Selective recording / Decision visualization（Tier 1-5）/ Context ref を含める。
...
**Objective:** As a Spec 1 起票者および将来の v2 開発者・実装者・保守者, I want frontmatter の共通必須・推奨・任意フィールドと wiki / review / skill / vocabulary / follow-up / hypothesis / perspective 各種のスキーマ骨格を Foundation で参照できる, so that 個別フィールドの実装は Spec 1 に委ねつつ、スキーマ全体像と意味的役割は spec 横断で固定される。
...
4. The Foundation shall Review レイヤー固有（`review/synthesis_candidates/` / `review/audit_candidates/` / `review/relation_candidates/` 等を含む `review/*_candidates/` 全般、および `review/decision-views/`（Decision log の Tier 2 markdown timeline 出力先、`_candidates` 命名規則の対象外））/ Skill ファイル / Skill candidate / Vocabulary candidate / Follow-up / Hypothesis candidate / Perspective 保存版の各スキーマ骨格を一覧として位置付け、詳細なフィールド仕様および candidate サブディレクトリの完全な列挙は Spec 1 に委譲する旨を記述する。
5. While L2 Graph Ledger の記録形式を記述する場合, the Foundation shall `entities.yaml`（正規化エンティティ）/ `edges.jsonl`（edge 正本）/ `edge_events.jsonl`（confidence 変遷 append-only）/ `evidence.jsonl`（根拠引用集 first-class、§2.10 trust chain 縦軸）/ `rejected_edges.jsonl`（reject 履歴）/ `decision_log.jsonl`（curator decision rationale、§2.13 curation provenance 横軸）の 6 ファイルが core schema を構成することを明示し、`decision_log.jsonl` は §2.10 evidence chain と直交する WHY 保全媒体として独立に位置付ける。
...
**Objective:** As a Spec 4 起票者および将来の v2 開発者・実装者・保守者, I want Foundation で Task 一覧と Command 一覧を上位カタログとして固定する, so that 個別 CLI の実装は Spec 4 に委ねつつ、コマンド名・実行モード・対話ガイド有無の対応が spec 横断で参照できる。
...
2. The Foundation shall Command 一覧をコア / Input Pipeline / Knowledge Generation / Approval / Query / Audit / L2 Graph Ledger 管理 / Edge 個別操作 / Entity-Relation 抽出 / Reject workflow / Page Lifecycle / Tag Vocabulary / Skill Library / Follow-up / Vault 管理 / Decision Log（Curation Provenance、`rw decision history` / `recent` / `stats` / `search` / `contradictions` / `render` 等を統括）のカテゴリ別に整理する。
...
7. The Foundation shall Spec 1-7 が Foundation 用語・原則・マトリクスを引用する際の **整合性検証手段** が必要であることを明示し、その実現手段として次のいずれか以上を design phase で確定すべきことを要件として残す: (a) 用語集語彙が Spec 1-7 から引用された箇所の link 切れ検出、(b) Foundation 改版時の影響範囲を decision_log.jsonl の `decision_type: "foundation_revision"` として §2.13 の枠組みで記録、(c) Adjacent Spec Synchronization 適用時の波及範囲（影響を受ける spec 一覧）の機械生成。SSoT は「定義の存在」だけでは機能せず「引用の正しさ」の検証手段が伴って初めて機能する。
8. If Spec 1-7 が Foundation の SSoT に違反した記述（用語の独自再定義、原則の独自再解釈、マトリクスの独自上書き等）を含むことが検出された場合, then the Foundation shall その違反は **先に Foundation 改版を要する変更提案** として扱い、Spec 1-7 単独での再解釈承認は行わないことを示す（R3.8 の拡張手順と整合）。

### Requirement 11: 実装独立性と運用前提

**Objective:** As a Spec 2 / Spec 3 / Spec 4 起票者および将来の v2 開発者・実装者・保守者, I want LLM CLI とエディタへの実装独立性、および運用上の前提（Python 3.10+ / Git 必須 / Concurrency lock 等）を Foundation で固定する, so that 個別 spec が特定 LLM CLI / 特定エディタに固有の前提を埋め込むことを防ぐ。
...
**Objective:** As a Spec 5 / Spec 6 起票者および将来の v2 開発者・実装者・保守者, I want §2.13 Curation Provenance の原則が Foundation で固定されている, so that decision_log.jsonl の存在と「what + why」の二層化が個別 spec で見落とされない。
...
**Objective:** As a Spec 5 / Spec 7 起票者および将来の v2 開発者・実装者・保守者, I want Git を第一の履歴ソースとしつつ、層ごとに補助履歴媒体が異なるという §2.6 原則を Foundation で固定する, so that 各 spec が層に不適合な履歴媒体（例: L2 で frontmatter update_history を使う等）を採用しない。
...
5. While L2 edge reject の必須記録項目を記述する場合, the Foundation shall `edge_id` / `rejected_at` / `reject_reason_category`（6 種定型）/ `reject_reason_text`（空文字禁止）/ `rejected_by` / `pre_reject_status` / `pre_reject_evidence_ids` / `pre_reject_confidence`（reject 直前 confidence 値、float [0.0, 1.0]、unreject 時の復元クランプ計算に使用、Spec 5 Requirement 12.4 / 12.7 と整合）を必須フィールドとして列挙する。
...
- 2026-04-26: 初版生成 + 4 段階レビュー反映 + approve（指摘 17 件、`decision_log.jsonl` を L2 core schema 追加 / `rw discover` 規範化 / `review/decision-views/` 骨格追加 / 13 原則拡張手順規範化）
- 2026-04-27: Spec 5 第 2 ラウンド由来 Adjacent Sync — Requirement 13.5 の `rejected_edges.jsonl` 必須フィールドを 7 → 8 に拡張し `pre_reject_confidence`（reject 直前 confidence 値、float [0.0, 1.0]）を追加（drafts §5.10 example の `original_confidence` と整合、Spec 5 R12.7 unreject 復元の決定的クランプ計算に必要、roadmap.md「Adjacent Spec Synchronization」運用ルールに従い再 approval 不要）
