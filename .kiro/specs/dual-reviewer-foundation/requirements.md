...
- ドラフト v0.2 = `.kiro/drafts/dual-reviewer-draft.md` §2.1 / §2.6 / §2.9 / §2.10.3 / §3 / §4
...
1. The 共通 JSON schema shall `review_case` object を以下 field で定義する: id / phase / timestamp / round / primary_findings / adversarial_findings / integration_result / trigger_state (`trigger_state` は review_case = run level の実行制御状態であり、finding level の `miss_type` / `difference_type` とは異なる粒度)
2. The 共通 JSON schema shall `finding` object を以下 field で定義する: id / round / origin / description / impact_score / miss_type / difference_type (severity は impact_score 内に統合する、draft v0.2 §2.6「既存 severity を 3 軸 impact_score に拡張」と整合)
...
4. The seed_patterns.yaml shall Layer 1 framework が定義する pattern schema (中程度 granularity + primary_group + secondary_groups) に準拠し、domain tag は付与しない (8 メタ群 + 中程度 granularity で domain 横断、固有性は concrete レベルのみ、draft v0.2 §2.9 規範)
