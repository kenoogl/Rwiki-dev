...
- [ ] Spec 6 implementation phase 中の rework 記録 (Data 1 + Data 2 + Data 3 全有効、§4 A-2.3 参照)
...
**cost 見積**: 1 work day = 6-10 時間 (= compact code 想定、reverse engineering + V4 適用 + behavior 差分検証 + raw data 抽出)。
...
- [ ] re-implementation 生成 + 既存コードとの behavior 差分検証
- [ ] per-finding raw data + per-system metrics + spec characteristic descriptive metric (= §3.7.6.1 と同 schema)

**cost 見積**: 1.5-2 work day = 10-14 時間 (= geometry / mesh / I/O の追加複雑度を反映)。
...
- [ ] 既存コード → reverse engineering で spec 起草
- [ ] V4 protocol で req phase + design phase review 取得
- [ ] re-implementation 生成 + 既存コードとの behavior 差分検証 (= sensor reading 精度 / interrupt latency 等の behavior 軸)
- [ ] per-finding raw data + per-system metrics + spec characteristic descriptive metric (= §3.7.6.1 と同 schema)

**cost 見積**: 0.5-1 work day = 4-8 時間 (= compact embedded 想定)。
...
**§3.7.6 全体 cost 見積**: §3.7.6.1 (6-10h) + §3.7.6.2 (10-14h) + §3.7.6.3 (4-8h) = **20-32 時間 ≈ 3-5 work day = A-2 完走後 1 calendar 月 batch**。着手順序: §3.7.6.1 (Phase field、最 compact) 先行で reverse-engineering 方式 feasibility 検証 → §3.7.6.2 → §3.7.6.3 と cascade。
...
#### A-2.3 Spec 6 Impl phase (= passive、Level 6 rework signal 観測、Claim D evidence)

Spec 6 implementation phase 中の上流 artifact (req/design/tasks) への post-approve 改版を `rework_log.jsonl` に passive 観測。dual-reviewer skill 適用なし、通常の wiki 機能実装 (= perspective-generation 機能の実装作業) を進行しながら、§3.6 Data 2 operational guide (v1.2) に従って append。

- [ ] Spec 6 implementation phase 中の rework 記録 (= Data 1 自動 + Data 2 manual + Data 3 任意 = §3.6 Data 2 operational guide v1.2 整合)
- [ ] Spec 6 完走時点の within-spec rework 内訳分析 (M1 / M2 / M3 / M4 比率算出)
