...
_v1.8 / 2026-05-04 (40th セッション末、treatment=single 系統 10 round 完走 = A-2.1 1/3 段階完了 = §A-2.1 第 2 系統 checkbox `[x]` 完走反映 + 累計 metric (46 detect / 17 採用 / 29 skip / 過剰修正比率 63.0% / Level 6 events 17 件) + treatment-single endpoint `33e1a12` 反映 = evidence-catalog v0.11 §5.2.1.6 整合化)_

_v1.9 / 2026-05-04 (45th セッション末、treatment=dual 系統 10 round 完走 = **A-2.1 全 3 系統完走 = 3 系統対照実験終端** = §A-2.1 第 3 系統 checkbox `[x]` 完走反映 + 累計 metric (60 detect / 40 rework events / 過剰修正比率 21.7% / Level 6 events 40 件) + treatment-dual endpoint `1b15138` 反映 = evidence-catalog v0.12 §5.2.1.7 整合化)_

_v1.7 / 2026-05-03 (30th セッション初頭、3 系統対照実験 design.md state policy 明示 = pristine state 起点 (commit `285e762`) + 各 treatment 独立 branch 戦略 = paper rigor 確保のための confounding 排除)_

_v1.6 / 2026-05-03 (29th セッション末、A-2.3 critical path 外し + §3.7.6 re-impl phase で Claim D primary evidence 代替 = §3.7.6 Level 6 観測統合 + 6 spec instance for Claim D primary evidence)_
...
- **軸 5 (v1.5 新設、ground truth availability)**: 既存コード reference の有無による evidence rigor 軸
  - 構成: forward-fresh-spec sample (= reference impl 不在、§3.7.1 / A-1 / A-2) vs code-derived spec sample (= reference impl 存在、§3.7.6 新設) の 2 状態
  - 主張範囲: 「reference impl 存在 sample で re-implementation behavior 差分が dual-reviewer review 結果と相関する場合、ground truth-anchored 補強 evidence として functional」
  - **避ける**: 「ground truth available = 真の defect 全て検出」claim (= reverse-engineering bias で既存コードの盲点が spec に転写される、軸 6 と同時考慮)
- **軸 6 (v1.5 新設、spec source: forward-fresh vs reverse-engineered)**: spec 起草方式の bias profile 軸
  - 構成: forward-fresh-spec (= A-1 / A-2 / §3.7.1、greenfield 起草、reference impl 不在) vs reverse-engineered (= §3.7.6、既存コード → spec → re-impl、reference impl 既存) の 2 状態
  - 主張範囲: 「forward / reverse 両方で dual-reviewer の bias correction 効果が類似 pattern を示す → 効果が spec construction 方式に robust」または「systematic 差異がある → spec construction 方式が moderating variable」
  - **避ける**: 「reverse 結果を forward に直接 generalize」claim (= reverse-engineering 5 bias source = spec completeness / convergent thinking / design choice rationalization / edge case asymmetry / implementation detail leakage、§5 Constraints + sub-group analysis 必須)
- **軸 7 (v1.5 新設、言語 diversity)**: LLM の言語 capability 由来 confound 検証軸
  - 構成: Python (= A-1 / A-2 / §3.7.1) vs C++ / Julia / Arduino C (= §3.7.6) の 4 言語
  - 主張範囲: 「dual-reviewer の bias correction 効果が複数言語で類似 pattern → 効果が言語 specific でない (= Python-trained LLM 偏向の反証)」
  - **避ける**: 「LLM tooling support が薄い言語 (= Arduino C / Julia) でも Python と完全等価 performance」claim (= 言語特異 confound は §5 Constraints で acknowledge、sub-group analysis で per-language metric report)

---

## §3 データ取得 checklist (6 levels + 2 triangulation batch、v1.5 拡張)

_Level 1-6 = spec ごとの per-finding evidence pipeline (raw data → metrics → hypothesis → figure → fork judgment → rework signal)。§3.7 = forward-fresh-spec + cross-LLM + mutation + multi-run の triangulation evidence batch (v1.1 新設、軸 4 整合)。§3.7.6 = code-derived spec batch (v1.5 新設、軸 5+6+7 整合 = ground truth availability + forward-reverse spec source + 言語 diversity)。§3.7 と §3.7.6 は両方必須 = forward / reverse spec の sub-group 比較が paper rigor の core defense (= reverse-engineering 5 bias source への mitigation)。_
...
**記録範囲**: A-1 implementation phase 着手 (= 15th セッション、本 v1.0 改版完了直後) ~ A-3 + §3.7.6 完走 (= Phase A 終端、v1.6 redefine = §3.7.6 re-impl phase 統合) の期間で発生する全 rework 事象。

**Claim D primary evidence 構成 (v1.6 redefine、A-2.3 critical path 外し + §3.7.6 で代替)**:

- **A-1 implementation phase**: 3 spec (foundation + design-review + dogfeeding) = self-dogfooding、Python、forward-fresh-spec、現状 0 events (29th 末確定)
- **§3.7.6 re-implementation phase** (v1.6 統合): 3 sample (Phase field 法 / 3D 熱伝導方程式 / Arduino IoT) = external project、Julia or C++ / Julia or C++ / Arduino C、reverse-engineered-spec、re-impl phase で発生する rework events を Level 6 schema で観測
- = **合計 6 spec instance for Claim D primary evidence** (= sample size 倍増 + cross-project transfer + 言語 diversity + ground truth-anchored 補強)
- **A-2.3 Spec 6 implementation phase** (v1.6 update): critical path から外し、Phase B-1.x post-paper supplementary evidence として defer (= Spec 6 implementation 完了時点で paper revision 用補強 data として後付け、論文 draft 着手の前提条件にしない)
...
- 「気づきはあったが上流改版は不要と判断したケース」: 即時 append 不要、implementer の運用記憶に保持し、後段の cross-spec review で formal change 候補として再評価
...
- **B3. A-1 vs §3.7.6 cross-project + cross-language 比較 (v1.6 update from "A-1 vs A-2 within-project")**: 同 V4 protocol で 3 spec (A-1 self-dogfooding、Python、forward-fresh) vs 3 sample (§3.7.6 external project、Julia/C++/Arduino C、reverse-engineered) → V4 reproducibility 補強 + cross-project transfer + 言語 diversity + ground truth-anchored 軸同時取得。Sample size 均衡 (= 3 vs 3) で sub-group analysis 容易。
- **caveat (v1.6 update)**: Sample size 制約 (A-1 = 3 spec + §3.7.6 = 3 sample = 6 spec instance) = quantitative 統計より qualitative pattern (どんな手戻りがどこで発生したか) 抽出が現実的、paper limitations section 明示。**reverse-engineering 5 source bias** (= §5 Constraints + §3.7.6 全体 caveat 整合) は Claim D primary metric にも影響可能性 (= reverse spec で post-approve rework が forward 起草より少なく出る系統的偏向) = forward (A-1) と reverse (§3.7.6) を別 sub-group reporting で対処。**A-2.3 Spec 6 implementation = Phase B-1.x supplementary** (v1.6 update) = critical path 外、Spec 6 完走時点で paper revision 用補強 data として後付け。
- **B4. A-2.1 3 系統対照実験 design.md state policy (v1.7 新設、paper rigor 確保のための confounding 排除)**: A-2.1 Spec 6 design phase 30 review session (= 3 系統 × 10 round) の input design.md state は **全 treatment が pristine state (= commit `285e762`、19th sub-step 2 = 起草直後 state) を起点**として完走する。各 treatment は独立 branch (`treatment-single` / `treatment-dual`) に派生し、treatment 内 Round 1-10 累積修正を branch 内で完結。第 1 系統 (= main、`treatment=dual+judgment`) は 20-29th で `285e762` 起点 → `f6bac54` 終点で完走済 = main archive。第 2 系統 (`treatment=single`) + 第 3 系統 (`treatment=dual`) は 30th 以降に `285e762` を起点に new branch 派生して 10 round 完走。**confounding 排除根拠**: 累積修正 state を共有して各 treatment Round 1-10 を回すと「prior treatment による事前修正」と「current treatment 単独 effect」が交絡 = V4 各 layer の機能寄与を quantify 不能 + Claim B/C primary evidence の paper rigor 致命的弱体化 (= self-referential bias)。pristine state 起点で各 treatment 独立完走することで、3 系統間の検出件数 / 採択率 / 過剰修正比率 / Level 6 events / wall-clock の直接比較性が保証され、ablation study 形式の論文 figure (= V4 各 layer 寄与の visualisation) が成立する。**branch 管理規律**: 各 treatment branch は main と独立 commit history を保持、main 進行に影響なし、A-2.1 完走後に 3 branch 統合して comparison-report v0.2 final 集約。Spec 6 design.md は最終的に main の post-Round 10 state (= dual+judgment 結果) を採用、他 2 treatment の修正 history は archive branch として paper data source 用に保持。
...
##### A-2 dogfeeding (A-1 完走後、推定 1-2 month、v1.6 update = A-2.3 critical path 外し)

- [ ] **A-2.1 Spec 6 design phase 内 V4 review process events** (= adaptive 用法、Claim B/C functioning evidence、Claim D primary evidence ではない、§5.1.6 解釈整合化済): 29th 末 = 44 events (treatment=dual+judgment 全 10 ラウンド完走、A-2.1 1/3 終端)
- [ ] A-2.2 Spec 6 tasks phase ad-hoc V4 適用 events (option、補助 evidence)
- [ ] **A-2.3 Spec 6 implementation phase 中の rework 記録** (v1.6 = critical path 外し、Phase B-1.x post-paper supplementary evidence として defer): Spec 6 implementation 完了時点で後付け、論文 draft 着手の前提条件にしない
...
- [ ] **§3.7.6.1 Phase field 法 re-impl phase rework 記録** (= reverse-engineered、Julia or C++、cost 6-10h 元見積に Level 6 観測 +0-3h overhead)
...
- [ ] **A-3 + §3.7.6 完走時点 final analysis** (Phase A 終端、v1.6 redefine = A-2 終端ではない): A-1 (3 spec self-dogfooding、Python、forward-fresh) + §3.7.6 (3 sample external、Julia/C++/Arduino C、reverse-engineered) = 6 spec instance for Claim D primary evidence + sub-group analysis narrative + paper claim 文言確定
...
A-1 implementation phase Step 1-3 完走時点 (17th セッション末) で `rework_log.jsonl` = 0 events 状態は、Data 1 (commit pattern auto、`git log -- .kiro/specs/dual-reviewer-*/`) で機械検証済の **"0 upstream artifact rework events"** であり、論文化時の文言は以下のように precise に解釈される:

- **正しい主張**: "V4 review が機能した 3 spec の A-1 implementation phase で、上流 artifact (req/design/tasks) への post-approve 改版は **0 events** = Claim D の strong evidence (= V4 review が下流で破綻しない)"
...
### §3.7 A-3 Triangulation evidence batch (v1.1 新設 / v1.5 拡張、軸 4-7 = multi-indicator convergence の primary evidence)

**目的**: §1 Validation framing で確定した "convergent multi-indicator triangulation" の indicators を A-2 dogfeeding 完走後の独立 batch (= scenario B) として取得。v1.5 で軸 5+6+7 (= ground truth / forward-reverse spec / 言語 diversity) 対応の §3.7.6 を新設、§3.7.1 を forward-fresh-spec 軸の独立必須 sample として re-position。論文 reviewer 想定批判 1 (= self-referential metric) + 批判 2 (= sample size / domain transfer) + 批判 3 (= LLM-on-LLM bias) + 批判 5 (= order effect) + 批判 8 (= ecological validity) + 批判 9 (= 言語 generalization、v1.5 新規) の同時 mitigation。

**timing**: A-2 dogfeeding 完走後の独立 batch (= user 明示「分離した方が集中できる」per 16th 議論)。A-2 並行は集中度低下のため非推奨。Phase A 終端 redefine = A-3 完走 = 論文 draft 着手 timing (§4 + §6 整合)。**着手順序 (v1.5 確定)**: §3.7.6 先行 (= 3 reverse-engineered samples、軸 5+6+7 cover) → §3.7.1 (= forward-fresh-spec 軸独立 sample) → §3.7.2-5 (multi-vendor / mutation / multi-run / convergence judgment) の順。§3.7.6 先行で domain abstraction gap を即時 cover し、reverse-engineering 方式の feasibility を Phase field 先行で検証。

**累計 cost 見積 (v1.5 update)**: §3.7.1 (6-12h) + §3.7.2 (3-6h) + §3.7.3 (7-10h) + §3.7.4 (3-5h) + Level 6 (~0、既存 plan 内) + §3.7.6 (合計 2-4 work day = 16-32h、3 sample 集計) + §3.7.7 future work (= scope 外、議論記録のみ) = **35-65 時間 ≈ 5-8 work day = A-2 完走後 1.5-2.5 calendar 月 batch** (v1.1 比 + 16-32h、§3.7.6 3 sample 追加分)。

**批判 mitigation 予想 (v1.5 update)**: 批判 1 (~85%、v1.1 80% + reverse spec の ground truth-anchored 補強) + 批判 2 (~80%、v1.1 70% + domain abstraction gap 解消) + 批判 3 (~70%、v1.1 同) + 批判 5 (~80%、v1.1 同) + 批判 8 (~90%、v1.1 同) + 批判 9 (~75%、v1.5 新規 言語 generalization defense) = 論文 acceptable line 明確に内。

#### §3.7.1 A-3.1: フルスクラッチ project (cross-project transfer + forward-fresh-spec 独立 sample、軸 4-iv + 軸 6 forward 側)

**目的 (v1.5 re-position)**: dual-reviewer 同程度規模の別 domain project で req+design phase に V4 適用、(a) **cross-project transfer evidence** (= Spec selection bias 排除 + domain transfer 補強、軸 4-iv = 批判 2 + 批判 8 mitigation)、(b) **forward-fresh-spec 軸の独立必須 sample** (= reference impl 不在の greenfield 起草、軸 6 forward 側 = §3.7.6 reverse-engineered batch との sub-group 比較 base、批判 1 self-referential metric への direct defense)。本 entry は「保険」ではなく **forward-fresh-spec 軸の paper rigor core defense として必須**、§3.7.6 と sub-group 比較で reverse-engineering bias を分離可能化。
...
- [ ] **§3.7.6 reverse-engineered batch との sub-group 比較**: 全 metric を forward-fresh (= §3.7.1 + A-1 + A-2) vs reverse-engineered (= §3.7.6) で別計算 + aggregate 併記 (= v1.5 §7 運用規律 sub-group analysis 整合、軸 6 evidence)
- [ ] **spec characteristic descriptive metric 記録**: AC 数 / 文字数 / Design Decisions 数 / 言及 alternative 数 / 起草所要時間 (= reverse-engineering bias 5 source への定量化方法、§5 Constraints 整合)

**caveat (v1.5 update)**: フルスクラッチ project = 別 domain (= dual-reviewer 自身ではない) を選定、ただし規模 + 複雑度は dual-reviewer 同程度 (= ecological validity 担保)。Project selection bias 排除のため、A-3 着手前の brief 確定段階で domain + scope を fix し変更しない。**§3.7.6 着手後 timing**: §3.7.6 先行で reverse-engineering 方式の feasibility 確認後 (= Phase field sample 完走時点) に §3.7.1 brief 着手で並走可能、ただし spec construction 方式の独立性担保のため §3.7.6 と異なる person / session で起草することを推奨 (= author bias 軸の最低限 mitigation)。
...
#### §3.7.5 A-3 evidence convergence judgment (Phase A 終端 trigger、v1.5 update)

A-3.1-A-3.4 + 既存 Level 6 + §3.7.6 sub-group 比較 = 6 件 indicators の convergence を judgment、論文 draft 着手可否を確定:

- [ ] **convergent (Phase A 終端 + 論文 draft 着手 go)**: 6 件中 5 件以上が V4 有効性方向に converge (= multi-vendor agreement ≥ 70% AND mutation sensitivity ≥ 70% AND multi-run agreement ≥ 80% AND cross-project metric stability ≥ ±20% AND Level 6 M4a ≤ 40% AND forward-reverse sub-group 比較で systematic 差異が paper limitations 範囲内)
- [ ] **mixed (Phase A 終端 + 論文 draft 着手 go with limitations expanded)**: 6 件中 3-4 件 converge + 残り mixed (= 一部 indicator で V4 有効性方向に converge、他 indicator で nuanced)。論文 limitations section で各 indicator の disagreement narrative を明示。
- [ ] **non-convergent (Phase A hold + V4 protocol 改修要)**: 6 件中 ≤ 2 件 converge = V4 改修材料 + paper claim 縮小要、本 plan 改版要。
- [ ] convergence judgment 結果 + narrative を `comparison-report.md` 最終版に append (= section ID `triangulation-convergence-judgment-v1.5`)。

#### §3.7.6 A-3.6: Code-derived spec batch (v1.5 新設、軸 5+6+7 = ground truth + reverse-engineered spec + 言語 diversity)

**目的**: 既存コードを reverse engineer して spec を起草、その spec から re-implementation を生成して dual-reviewer review を取得。3 sample で軸 5 (ground truth availability) + 軸 6 (reverse-engineered spec source) + 軸 7 (言語 diversity = Python 以外) を同時 cover。批判 1 (= self-referential metric、ground truth-anchored 補強) + 批判 2 (= sample size / domain transfer) + 批判 8 (= ecological validity) + 批判 9 (= 言語 generalization、v1.5 新規) mitigation。

**位置付け**: §3.7.1 forward-fresh-spec 独立 sample との sub-group 比較が paper rigor の **core defense**。Reverse-engineering 5 bias source (= spec completeness / convergent thinking / design choice rationalization / edge case asymmetry / implementation detail leakage) を §5 Constraints + §7 運用規律 sub-group analysis で mitigation。

##### §3.7.6.1 Phase field 法 (compact、Julia or C++)

**コード特性**: 純粋数値計算、PDE numerical methods (= Allen-Cahn / Cahn-Hilliard 系)、コンパクト (= scope creep risk 低)、domain expertise 高 (= materials science + 数学厳密性)。Reverse-engineering 方式の feasibility 検証 sample (= 着手順序最先行) として最適。

- [ ] 既存 Phase field コード (Julia or C++) を選定 + scope fix
- [ ] 既存コード → reverse engineering で spec (brief / req / design) 起草
- [ ] V4 protocol で req phase + design phase review 取得 (= 1 spec instance 追加)
- [ ] re-implementation 生成 (= 既存コードと parallel reference impl)
- [ ] **re-impl phase 中の Level 6 rework events 観測** (v1.6 新設、Claim D primary evidence): post-approve upstream artifact (req/design/tasks) への改版 events を `rework_log.jsonl` に append、`spec_id="phase-field"` + `discovered_phase="impl-mid"` 付与、A-1 (forward-fresh) vs §3.7.6 (reverse-engineered) sub-group 比較 base
- [ ] re-impl vs 既存コード の behavior 差分検証 (= ground truth-anchored validation、軸 5 evidence)
- [ ] per-finding raw data (foundation Req 3 共通 schema、`miss_type` / `difference_type` / `trigger_state` 自己ラベリング、forward-spec / reverse-engineered explicit label)
- [ ] per-system metrics (= 検出件数 / 採択率 / 過剰修正比率 / wall-clock / V4 修正否定 prompt 機能)
- [ ] spec characteristic descriptive metric 記録 (= AC 数 / 文字数 / Design Decisions 数 / 言及 alternative 数 / 起草所要時間)
...
##### §3.7.6.2 3D 熱伝導方程式 + 複雑モデル生成 (Julia or C++)

**コード特性**: 数値計算 + geometry / mesh generation + I/O robustness、§3.7.6.1 より rich (= mesh 生成 / boundary condition / I/O integration の non-trivial concrete logic)。real-world numerical project に近い ecological validity。

- [ ] 既存 3D 熱伝導コード を選定 + scope fix (= 複雑モデル生成部分含む)
- [ ] 既存コード → reverse engineering で spec 起草
- [ ] V4 protocol で req phase + design phase review 取得
- [ ] re-implementation 生成 + **re-impl phase 中の Level 6 rework events 観測** (v1.6 新設、Claim D primary evidence、`spec_id="heat-equation-3d"` 付与) + 既存コードとの behavior 差分検証
- [ ] per-finding raw data + per-system metrics + spec characteristic descriptive metric (= §3.7.6.1 と同 schema)

**cost 見積 (v1.6 update)**: 1.5-2 work day = 10-14 時間 (= geometry / mesh / I/O の追加複雑度を反映) + Level 6 観測 overhead +0-3h。

##### §3.7.6.3 Arduino IoT センサ (Arduino C / C++)

**コード特性**: embedded systems、real-time / hardware constraint / power management / interrupt handling / sensor noise の failure mode、軸 2 (= concurrency / real-time) を numerical 2 sample 以外で cover。

- [ ] 既存 Arduino IoT コード を選定 + scope fix (= sensor + 通信 + interrupt 含む小規模 sketch)
- [ ] 既存コード → reverse engineering で spec 起草
- [ ] V4 protocol で req phase + design phase review 取得
- [ ] re-implementation 生成 + **re-impl phase 中の Level 6 rework events 観測** (v1.6 新設、Claim D primary evidence、`spec_id="arduino-iot"` 付与) + 既存コードとの behavior 差分検証 (= sensor reading 精度 / interrupt latency 等の behavior 軸)
- [ ] per-finding raw data + per-system metrics + spec characteristic descriptive metric (= §3.7.6.1 と同 schema)

**cost 見積 (v1.6 update)**: 0.5-1 work day = 4-8 時間 (= compact embedded 想定) + Level 6 観測 overhead +0-3h。

##### §3.7.6 全体 caveat + Reverse-engineering bias mitigation (5 step、v1.5 新設)

**caveat (5 reverse-engineering bias source)**:

- (1) **spec completeness bias**: spec writer が既存実装を「見て」起草するため forward 起草より過剰に完全化、ambiguity / open question が forward 起草の自然な所産だが reverse では既に解消済
- (2) **convergent thinking bias**: spec 起草が「既存実装の説明」化、設計探索 (= alternative 検討、却下案記録) が極小化、research.md の Decisions section が貧弱化
- (3) **design choice rationalization**: 既存コードの設計選択を後付け justification 化、forward なら異なる選択 path を取った可能性が見えなくなる
- (4) **edge case asymmetry**: 既存コードに existing なら spec が拾う / 既存コードに missing なら spec も missing (= 既存コードのバグ・盲点が spec に転写)
- (5) **implementation detail leakage**: spec が forward 規範では記述すべきでない実装詳細 (= 内部 algorithm 選択、特定 data structure) を含む傾向

**Mitigation 5 step (minimum viable approach、§7 運用規律 sub-group analysis 整合)**:

- **Step 1**: 全 sample を **forward-spec / reverse-engineered で explicit label** (= raw data JSONL の per-finding entry に label field 追加、後で grep 可能)
- **Step 2**: 各 sample で **spec characteristic descriptive metric を記録** (= AC 数 / 文字数 / Design Decisions 数 / 言及 alternative 数 / 起草所要時間、上記 §3.7.6.1-3 entry で AC 化)
- **Step 3**: dual-reviewer 全 metric を **sub-group reporting** (= forward 集合 = §3.7.1 + A-1 + A-2 / reverse 集合 = §3.7.6 で別計算 + aggregate 併記、論文 figure 1-7 すべてで sub-group 区別)
- **Step 4**: paper Limitations / Threats to Validity セクション内に **reverse-engineering bias sub-section** 配置 (= 上記 5 source + 3 mitigation 措置 (explicit labeling + sub-group reporting + spec characteristic metric) を 1-2 段落)
- **Step 5**: paired comparison は scope 外として論文に明記、Future Work で言及 (= §3.7.7 future work 整合、optional cost-overhead 大で paper add value 限定的)

**§3.7.6 全体 cost 見積 (v1.6 update)**: §3.7.6.1 (6-10h + Level 6 観測 0-3h) + §3.7.6.2 (10-14h + 0-3h) + §3.7.6.3 (4-8h + 0-3h) = **20-41 時間 ≈ 3-6 work day = A-2 完走後 1 calendar 月 batch**。**Level 6 rework events 観測を re-impl phase に統合** (v1.6 新設) = Claim D primary evidence の主要 source、A-2.3 critical path 外しの代替 = 6 spec instance 累計 (A-1 3 spec + §3.7.6 3 sample) + cross-project transfer + 言語 diversity 同時取得。着手順序: §3.7.6.1 (Phase field、最 compact) 先行で reverse-engineering 方式 feasibility 検証 → §3.7.6.2 → §3.7.6.3 と cascade。

#### §3.7.7 A-3.7: Paired comparison (future work、v1.5 新設、議論記録 only)

**目的 (議論記録 only、本 plan scope 外)**: 同一 problem を forward-spec (= 既存コード未参照) と reverse-engineered (= 既存コード参照) の両方で実施し、reverse-engineering bias を strict internal validity で disentangle。reverse-engineering 5 bias source の最強 mitigation (= gold standard) として標準 SE empirical literature (Wohlin et al.) で推奨される手法。

- 議論記録: 19th セッションで「reverse-engineering bias の paper limitations 記述方針」深掘り議論で identified
- scope 外理由: cost 2 倍 (= 同 problem を 2 通りで実施)、paper add value は §3.7.6 + §3.7.1 sub-group 比較で十分 acceptable line 内 mitigation 達成 (= §3.7.6 全体 caveat の 5 step minimum viable approach で paper rigor 確保)
- 採用条件: §3.7.6 sub-group 比較で systematic 差異が paper limitations 範囲外 = forward / reverse の混合で aggregate metric が confound する場合のみ、Phase B 以降で 1 paired sample を追加検討
- paper 記述方針: limitations section 内 Future Work 段落で「paired comparison として 1 sample で forward / reverse 両方を実施することは reverse-engineering bias の strict internal validity で disentangle 可能だが、本論文 scope では descriptive metric + sub-group analysis で十分な mitigation 達成、paired comparison は future work」と明記

**cost 見積 (= 採用時のみ)**: 1 sample = 1 work day = 6-10 時間 × 2 (forward + reverse 両方) = 12-20 時間。本 plan scope 外。
...
A-2 phase = Spec 6 (`rwiki-v2-perspective-generation`) を題材に **3 phase 段構成**の evidence acquisition を実施 (v1.3 整合化)。Spec 6 Design = systematic main / Tasks = ad-hoc V4 補助 (option) / Impl = Level 6 passive 観測 (Claim D)。

#### A-2.1 Spec 6 Design phase (= 主要 evidence、systematic、必須)

dual-reviewer skill (= 17th 末完成) を Spec 6 design.md に実適用、論文 figure 1-4 の主要 evidence を取得。

##### branch 戦略 (v1.7 新設、§3.6 baseline 制約 B4 整合、paper rigor 確保のための confounding 排除)

- 全 treatment は **pristine state (= commit `285e762`、19th sub-step 2 = 起草直後 state) を起点**として完走、累積修正 state の共有禁止 (= confounding 排除)
- **第 1 系統 (= `treatment=dual+judgment`、main branch)**: 20-29th で `285e762` 起点 → `f6bac54` 終点で完走済 = main archive (10 commit、Level 6 events 44 件累積)
- **第 2 系統 (= `treatment=single`)**: 30th 以降に main から `285e762` を checkout、new branch `treatment-single` を派生、10 round 完走 (推定 1 month batch)、completion 時に Level 6 events を rework_log.jsonl 別 file (= `rework_log_treatment_single.jsonl`) または別 sub-group key で記録 (= main の rework_log.jsonl と confound 回避)
- **第 3 系統 (= `treatment=dual`)**: 第 2 系統完走後に同様に `285e762` から new branch `treatment-dual` を派生、10 round 完走 (推定 1 month batch)、Level 6 events を別 file/sub-group key で記録
- A-2.1 完走後に 3 branch 統合分析 = comparison-report v0.2 final 集約 (= 3 系統間の primary 検出 / 採択率 / 過剰修正比率 / Level 6 events / wall-clock 直接比較表 + ablation figure data 生成)
- Spec 6 design.md 最終 state は main の post-Round 10 state (= `f6bac54`) を採用、他 2 treatment branch は paper data source archive として保持 (= delete 禁止)

- [ ] Spec 6 design への dual-reviewer 適用 (dr-init bootstrap + 3 系統 30 review session、各 treatment は pristine state `285e762` 起点 = §3.6 B4 整合)
- [x] **第 1 系統 (`treatment=dual+judgment`、main)** = 完走済 (29th 末、Round 1-10、commits `6e26aa8` ~ `f6bac54`、Level 6 events 44 件)
- [x] **第 2 系統 (`treatment=single`、treatment-single branch)** = 完走済 (40th 末、Round 1-10、treatment-single branch endpoint `33e1a12`、累計 46 detect / 17 採用 / 29 skip / 過剰修正比率 63.0% / Level 6 events 17 件 / 観点 axis 別 escalate 出現 pattern Round 1 0% → Round 10 40% suppress / forward Adjacent Sync 規律遵守完全実証 / 既存節拡張 vs 新節新設同型処置 pattern 確立 = evidence-catalog v0.11 §5.2.1.6 で詳細記録)
- [x] **第 3 系統 (`treatment=dual`)** = 完走済 (41st-45th、treatment-dual branch endpoint `1b15138` / `86cdc5f`、累計 60 detect / 40 rework events / 過剰修正比率 21.7% / Level 6 events 40 件 / 観点 axis 別 adversarial 独立補完 5 度目連続再現 / forced_divergence 14 件 / P+A 横断同型重複 4 度 = evidence-catalog v0.12 §5.2.1.7 詳細参照)
...
- [ ] Spec 6 design approve (= main の post-Round 10 state `f6bac54` を採用、他 2 treatment branch は archive として保持)

#### A-2.2 Spec 6 Tasks phase (= 補助 evidence、V4 ad-hoc 適用、option)

V4 protocol を Spec 6 tasks.md 生成時に手動再現適用、phase 横断 reproducibility 補助 evidence を 4 spec 目 tasks phase で拡張取得。dr-tasks skill 不在 (= Phase B-1.1 で別 spec 化予定) のため manual 適用、推定 cost 数時間-1日。dual-reviewer skill 自体は適用せず、V4 protocol §4 (forced_divergence + Step 1c judgment) のみ手動再現。

- [ ] Spec 6 tasks.md 生成 (`/kiro-spec-tasks rwiki-v2-perspective-generation` Skill 経由)
- [ ] V4 ad-hoc 適用 (= 14th 同様、forced_divergence + Step 1c judgment 手動再現)
- [ ] tasks phase metric 算出 (= 採択率 / 過剰修正比率 / V4 修正否定率)
- [ ] Spec 6 tasks approve

#### A-2.3 Spec 6 Impl phase (= passive、Level 6 rework signal 観測、v1.6 update = Phase B-1.x supplementary defer)

**v1.6 update**: A-2.3 を **critical path から外し、Phase B-1.x post-paper supplementary evidence として defer** = Spec 6 implementation 完了時点で paper revision 用補強 data として後付け、論文 draft 着手の前提条件にしない。Claim D primary evidence は **A-1 (3 spec self-dogfooding) + §3.7.6 (3 sample reverse-engineered re-impl phase) = 6 spec instance** で構成 (= §3.6 Claim D primary evidence 構成 + §4 §3.7.6 entry 整合)。

Spec 6 implementation phase 中の上流 artifact (req/design/tasks) への post-approve 改版は `rework_log.jsonl` に passive 観測継続 (= 既存運用維持)、ただし **paper draft 着手の critical path には含めない** (v1.6 redefine)。dual-reviewer skill 適用なし、通常の wiki 機能実装 (= perspective-generation 機能の実装作業) を進行しながら、§3.6 Data 2 operational guide (v1.2) に従って append。

- [ ] Spec 6 implementation phase 中の rework 記録 (= Data 1 自動 + Data 2 manual + Data 3 任意 = §3.6 Data 2 operational guide v1.2 整合) [Phase B-1.x supplementary]
- [ ] Spec 6 完走時点の within-spec rework 内訳分析 (M1 / M2 / M3 / M4 比率算出) [Phase B-1.x supplementary]
- [ ] A-1 (3 spec impl) vs §3.7.6 (3 sample re-impl) cross-project 比較 (v1.6 redefine、§3.6 B3 baseline 整合) ← critical path、§3.7.6 entry で primary evidence
- [ ] A-2.3 Spec 6 impl 完了時の paper revision supplementary data 後付け [Phase B-1.x]
...
### A-3 triangulation evidence batch (⏸️ A-2 完走後、推定 1.5-2.5 calendar 月、v1.1 新設 / v1.5 拡張)

§3.7 + 軸 4-7 整合の独立 batch (= scenario B、user 明示「分離した方が集中できる」per 16th 議論)。論文 reviewer 想定批判 1+2+3+5+8+9 同時 mitigation evidence。Phase A 終端 redefine (= v1.0 では A-2 完走 = Phase A 終端、v1.1 では A-3 完走、v1.5 では A-3 + §3.7.6 完走 = Phase A 終端)。

**着手順序 (v1.5 確定)**: §3.7.6 先行 → §3.7.1 (forward-fresh-spec) → §3.7.2-5 (multi-vendor / mutation / multi-run / convergence judgment)。§3.7.6 先行で domain abstraction gap を即時 cover し、reverse-engineering 方式 feasibility を Phase field sample で検証。§3.7.1 は §3.7.6 着手後に並走可能 (= sub-group 比較 base としての必須 sample)。

- [ ] **§3.7.6.1 Phase field 法 (compact、Julia or C++)** (cost 6-10h、§3.7.6.1) ← **A-3 batch 先頭、reverse-engineering feasibility 検証 sample**
  - sub-step: 既存コード選定 → reverse engineering で spec 起草 → V4 適用 → re-impl 生成 → behavior 差分検証 → metric 抽出
- [ ] **§3.7.6.2 3D 熱伝導方程式 + 複雑モデル生成** (cost 10-14h、§3.7.6.2)
  - sub-step: 既存コード選定 (= 複雑モデル生成含む) → reverse engineering → V4 適用 → re-impl + behavior 差分検証 → metric 抽出
- [ ] **§3.7.6.3 Arduino IoT センサ** (cost 4-8h、§3.7.6.3)
  - sub-step: 既存 Arduino sketch 選定 → reverse engineering → V4 適用 → re-impl + behavior 差分検証 → metric 抽出
- [ ] §3.7.1 フルスクラッチ project (= forward-fresh-spec 軸独立必須 sample) の req+design phase V4 適用 (cost 6-12h)
  - sub-step: brief 着手 (= §3.7.6 着手後並走可、user 任意 timing) → req phase V4 → design phase V4 → metric 抽出 → §3.7.6 sub-group 比較
- [ ] §3.7.2 multi-vendor LLM cross-validation (cost 3-6h)
  - sub-step: GPT-4 judgment + Gemini judgment 投入 → agreement matrix 算出 → disagreement qualitative analysis
- [ ] §3.7.3 mutation testing (cost 7-10h)
  - sub-step: mutation set 設計 (5-10 件) → バリアント生成 → V4 適用 × N+1 → sensitivity/specificity 集計
- [ ] §3.7.4 multi-run reliability check (cost 3-5h)
...
- [ ] A-3 evidence convergence judgment (§3.7.5) + `comparison-report.md` 最終版 append (= 6 indicators convergence、forward-reverse sub-group 比較含む)
- [ ] **最終 comparison-report v0.2** (= Phase A 終端時、req + design + A-2 + A-3 + §3.7.6 累計集計、triangulation evidence narrative + claim convergence/mixed/non-convergent verdict + forward-reverse sub-group 比較 narrative)
- [ ] **Phase A 終端 = A-3 + §3.7.6 完走** = 論文 draft 着手 timing (v1.5 redefine)

**累計 cost (v1.5 update)**: 35-65 時間 ≈ 5-8 work day = A-2 完走後 1.5-2.5 calendar 月 batch (v1.1 比 + 16-32h、§3.7.6 3 sample 追加分)。
...
- **Reverse-engineering bias 5 source caveats** (v1.5 新設、§3.7.6 entry 整合、paper Limitations section sub-section 配置必須):
  - (1) **spec completeness bias**: spec writer が既存実装を参照して起草するため forward 起草より過剰に完全化、ambiguity / open question が forward の自然所産だが reverse では既に解消済 → 期待される影響 = primary completeness bias の発動 surface 縮小 = 検出件数 / 過剰修正比率 reduction
  - (2) **convergent thinking bias**: spec 起草が「既存実装の説明」化、設計探索 (= alternative 検討、却下案記録) が極小化 → research.md Decisions section が貧弱化 = adversarial counter-evidence 検出 surface 縮小
  - (3) **design choice rationalization**: 既存コードの設計選択を後付け justification 化、forward なら異なる選択 path を取った可能性が見えなくなる → judgment override surface 縮小 = override rate reduction
  - (4) **edge case asymmetry**: 既存コードに existing なら spec が拾う / 既存コードに missing なら spec も missing (= 既存コードのバグ・盲点が spec に転写) → 既存コードに internal validity 制約あり (= dual-reviewer 検出可能 defect set が既存コードの盲点で限定)
  - (5) **implementation detail leakage**: spec が forward 規範では記述すべきでない実装詳細 (= 内部 algorithm 選択、特定 data structure) を含む傾向 → forward と reverse spec characteristic が異なる、spec characteristic descriptive metric (= AC 数 / 文字数 / Design Decisions 数 / 言及 alternative 数 / 起草所要時間) で定量化
  - **mitigation**: §7 運用規律 sub-group analysis 規律で全 metric を forward / reverse 別 reporting + spec characteristic descriptive metric 記録 + paper Limitations sub-section で 5 source 明示 (= 3.7.6 全体 caveat の Mitigation 5 step、論文 acceptable line 内 mitigation 達成)
  - **paired comparison (= gold standard) は scope 外**: §3.7.7 future work entry で議論記録のみ、Phase B 以降に必要性再判断
...
- **Self-review skill skip 規律 (v1.4 新設、採取軸保護、19th セッションで identified)**:
  - **対象**: design.md / requirements.md / tasks.md 起草直後に同 session の orchestrator が `/kiro-validate-design` / `/kiro-validate-gap` / `/kiro-validate-impl` 等の self-review skill を sequential 実行しない
  - **理由**: 起草者と reviewer が同一 LLM session = full context shared = self-review contamination で baseline として価値低い、かつ後続 dr-design 等の subagent fresh review 前提を歪める risk (orchestrator が contamination 結果を反映して artifact を修正すると pristine state が失われ、dual-reviewer Round 1 input が変質)
  - **影響範囲**: orchestrator level の context 汚染は subagent context に伝達されない (= subagent は独立 context で起動) ため damage は orchestrator level 限定。ただし orchestrator が artifact 修正 = 修正版が dr-design Round 1 input になる = pristine state 喪失の sequel risk
  - **例外**: (a) 別セッション or 別 subagent から validate-* 起動 = 採取軸外として実行可能 (= 本規律は同一 session 内 sequential invocation のみ対象) / (b) implementation phase 完了後の `/kiro-validate-impl` は dr-design 採取軸とは独立 phase = 適用対象外
  - **適用 phase**: A-2 phase で primary に適用 (= 19th セッション Spec 6 design.md 起草直後の validate-design 実行で identified)、A-3 batch のフルスクラッチ project でも同様、A-1 implementation phase 完了後は適用対象外
  - **違反時 handling**: (1) self-review 結果を artifact 修正に反映しない (= 破棄) / (2) artifact は pristine state 維持 / (3) 違反事実を session log に記録 (rework_log 対象外、self-report として記録) / (4) dr-design Round 1 は fresh subagent で起動して採取軸復旧
- **Sub-group analysis 規律 (v1.5 新設、軸 5+6+7 evidence 整合性保護)**:
  - **対象**: §3.7.6 reverse-engineered samples (= Phase field / 3D 熱伝導 / Arduino IoT) + §3.7.1 forward-fresh-spec sample + A-1 / A-2 forward-spec samples の sub-group 別 metric reporting
  - **explicit labeling 必須**: 全 sample (= raw data JSONL + per-system metric record + comparison-report entry) に `spec_source: forward-fresh | reverse-engineered` field を付与 = 後で grep 可能、論文 sub-group analysis の base
  - **sub-group reporting 必須**: dual-reviewer 全 metric (= 検出件数 / 採択率 / 過剰修正比率 / disagreement / judgment override / wall-clock / V4 修正否定 prompt 機能) を forward 集合 + reverse 集合 + aggregate の 3 形式で並列計算、論文 figure 1-7 すべてで sub-group 区別表記
  - **spec characteristic descriptive metric 必須**: 全 sample で AC 数 / 文字数 / Design Decisions 数 / 言及 alternative 数 / 起草所要時間 を記録 = reverse-engineering bias 5 source の定量化方法、forward / reverse の系統的差異が paper limitations section で報告可能
  - **paper Limitations sub-section 必須配置**: reverse-engineering bias 5 source + 3 mitigation 措置 (explicit labeling + sub-group reporting + spec characteristic metric) を Limitations / Threats to Validity 内 sub-section で 1-2 段落
  - **適用 phase**: §3.7.6 着手以降の全 A-3 batch sample、論文 draft 着手前に sub-group reporting 完成必須
  - **違反時 handling**: (1) sub-group 別計算なしの aggregate のみ report = paper rigor 不足、再計算で sub-group 別 reporting に修正 / (2) explicit labeling 漏れ sample は raw data 遡及で label 補完、不明な場合は除外
...
- **v1.2** (2026-05-01 18th セッション、Data 2 operational guide + framing 精緻化注記): user 指摘「rework_log = 0 は self-report か機械検証か」を契機に Data 2 の operational ambiguity を明文化。§3.6 に (a) **Data 2 operational guide** sub-section 新設 = 記録対象 3 件 + 記録対象外 6 件 + 判断保留時の処理 + implementer self-check 4 step、A-2 dogfeeding (Spec 6 適用) 着手前に Data 2 運用規律を確定し self-report の operational ambiguity を抑制 / (b) **v1.2 注記** = "0 upstream artifact rework events" の precise framing 明示、論文化時の overclaim ("rework が全くなかった") 防止 + Claim D primary metric 文言推奨 ("post-approve upstream artifact rework rate") + limitations 明記推奨。本 v1.2 改版自体は v1.0/v1.1 と同様 Level 6 記録対象外 (= methodology meta-document への改版で上流 spec artifact 改版 trigger なし)。
- **v1.3** (2026-05-02 18th セッション、A-2 phase 3 段構成展開 + §4 ⇄ §3.6 整合化): user 議論で「dual-reviewer は design phase 専用」誤認識を修正、A-2 phase の正確な scope を 3 phase 段構成として §4 Timeline に明示。**§4 A-2 dogfeeding entry を 3 sub-section + 終端 sub-section に展開** = (1) **A-2.1 Spec 6 Design phase** = dual-reviewer skill 30 session systematic、主要 evidence (= 論文 figure 1-4)、必須 / (2) **A-2.2 Spec 6 Tasks phase** = V4 protocol ad-hoc 手動再現 (forced_divergence + Step 1c judgment)、補助 evidence (= 14th 3 spec tasks phase の Spec 6 拡張、phase 横断 reproducibility)、option、cost 数時間-1日、dr-tasks skill 不在 (= Phase B-1.1 別 spec 化予定) / (3) **A-2.3 Spec 6 Impl phase** = Level 6 passive 観測 (Claim D evidence)、dual-reviewer skill 適用なし、通常実装作業並走 / (4) **A-2 phase 終端 + 統合分析** = A-1 vs A-2 比較 + Claim D 判断 + Spec 6 全 phase approve + Rwiki v2 全 8 spec design approve。**§3.6 checkbox tracker A-2 entry の wording を「Spec 6 dogfeeding 中の rework 記録」→「Spec 6 implementation phase 中の rework 記録」(§4 A-2.3 参照付き) に precise 化**、§4 と §3.6 の 2 場所間整合確保。本 v1.3 改版自体は Level 6 記録対象外 (= methodology meta-document)。
- **v1.4** (2026-05-02 19th セッション、Self-review skill skip 規律新設 + 採取軸保護): A-2 phase sub-step 2 = Spec 6 design.md 起草完了 (1146 行) 直後に user 起動 `/kiro-validate-design` で orchestrator (Opus) が同一 session 内 self-review を実行 (3 critical issue identified + NO-GO 判定)、user 指摘で採取軸違反を identified。validate-design 結果は破棄、design.md は pristine state 維持。**§7 運用規律 に Self-review skill skip 規律を新設** = 同一 session 内 sequential invocation 禁止 + 例外条件 2 件 (別 session/subagent + implementation 完了後 validate-impl) + 適用 phase (A-2 / A-3) + 違反時 handling 4 step。dr-design は fresh subagent context で起動するため subagent level での contamination は回避済 = damage は orchestrator level 限定。本規律は同種失敗の予防 = future session で design.md 起草直後の self-review 反射的実行を抑制する operational discipline。本 v1.4 改版自体は Level 6 記録対象外 (= methodology meta-document)。
- **v1.5** (2026-05-02 19th セッション末、軸 4 → 軸 7 拡張 + §3.7.6 Code-derived spec batch 新設 + §3.7.1 forward-fresh-spec 軸独立必須 sample に re-position + Sub-group analysis 規律新設): user 議論「dual-reviewer のサンプル数を増やす場合、異なる特性のコードで行った方がよい」を契機に、3 件の concrete code samples (= Phase field 法 + 3D 熱伝導方程式 + Arduino IoT センサ) を「既存コード → reverse engineering で spec 起草 → re-implementation」の方式で A-3 batch に組み込む方針を確定。**§2 比較軸 を 4 axes → 7 axes に拡張** = 軸 5 (ground truth availability) + 軸 6 (forward-fresh-spec vs reverse-engineered spec source) + 軸 7 (言語 diversity = Python / C++ / Julia / Arduino C) を新設。**§3.7.6 A-3.6 Code-derived spec batch を新設** = §3.7.6.1 Phase field 法 (compact、Julia or C++、cost 6-10h、reverse-engineering feasibility 検証 sample) + §3.7.6.2 3D 熱伝導方程式 + 複雑モデル生成 (cost 10-14h、real-world numerical project rich 化) + §3.7.6.3 Arduino IoT センサ (cost 4-8h、embedded systems failure mode cover) + 全体 caveat = reverse-engineering bias 5 source (spec completeness / convergent thinking / design choice rationalization / edge case asymmetry / implementation detail leakage) + Mitigation 5 step (explicit labeling + descriptive metric + sub-group reporting + paper Limitations sub-section + paired comparison defer)。**§3.7.1 を forward-fresh-spec 軸独立必須 sample に re-position** = 「保険」位置付けから sub-group 比較 base としての必須 sample に明確化 (= §3.7.6 と sub-group 比較で reverse-engineering bias を分離可能化)。**§3.7.7 paired comparison を future work entry として新設** (= 議論記録のみ、scope 外、Phase B 以降に必要性再判断)。**§3.7.5 convergence judgment を 5 indicators → 6 indicators に拡張** (= forward-reverse sub-group 比較を indicator として追加)。**§4 Timeline A-3 batch entry update** = 着手順序 §3.7.6 先行 → §3.7.1 → §3.7.2-5 を確定、累計 cost 19-33h → 35-65h (= §3.7.6 3 sample 追加分 +16-32h)、Phase A 終端 redefine = A-3 + §3.7.6 完走。**§5 Constraints に Reverse-engineering bias 5 source caveats 追加** + **§7 運用規律 に Sub-group analysis 規律新設** = explicit labeling + sub-group reporting + descriptive metric + Limitations sub-section + 違反時 handling。批判 mitigation 予想 update (批判 1 ~80% → ~85% / 批判 2 ~70% → ~80% / 批判 9 ~75% v1.5 新規) = 論文 acceptable line 内 mitigation 強化。本 v1.5 改版自体は Level 6 記録対象外 (= methodology meta-document)。
- **v1.9** (2026-05-04 45th セッション末、treatment=dual 系統 10 round 完走 = **A-2.1 全 3 系統完走 = 3 系統対照実験終端** 反映): treatment=dual branch (`treatment-dual`) 全 10 ラウンド完走 (= 41st-45th セッション累計、endpoint `1b15138` / log `86cdc5f`)。**§A-2.1 第 3 系統 checkbox `[ ]` → `[x]` 化** + treatment-dual 累計 metric inline 記録 (total detect 60 / rework events 40 / 過剰修正比率 21.7% / Level 6 events 40 件 / forced_divergence 14 件 + adversarial 独立補完 5 度目連続再現 + P+A 横断同型重複 4 度) + evidence-catalog v0.12 §5.2.1.7 詳細参照追記。**§A-2 全系統 checkbox 状況**: 第 1 系統 (dual+judgment、main) ✅ + 第 2 系統 (single、treatment-single) ✅ + 第 3 系統 (dual、treatment-dual) ✅ = **A-2.1 全 3 系統完走確定**。残 = comparison-report v0.3 final 集約 (`metric_extractor.py` + `figure_data_generator.py` + `phase_b_judgment.py` 実行) のみ。本 v1.9 改版自体は v1.8 同様 Level 6 記録対象外 (= methodology meta-document)。
- **v1.7** (2026-05-03 30th セッション初頭、3 系統対照実験 design.md state policy 明示 = pristine state 起点 + 各 treatment 独立 branch 戦略 = paper rigor 確保のための confounding 排除): 30th セッション開始時に user 指摘「single の Round 1 を開始するとき、元になる design.md はどうやって準備するか」を契機に、A-2.1 30 review session の input design.md state policy が SSoT 文書群に明示記述されていない盲点を identified。29th 末まで第 1 系統 (= main、`treatment=dual+judgment`) が pristine state `285e762` から 10 round 累積修正で `f6bac54` に到達、accumulated state を起点に第 2/第 3 系統を回すと「prior treatment による事前修正」と「current treatment 単独 effect」が交絡 = V4 各 layer の機能寄与を quantify 不能 + Claim B/C primary evidence の paper rigor 致命的弱体化 (= self-referential bias)。**§3.6 baseline 制約 B4 新設** = A-2.1 30 review session 全 treatment は pristine state `285e762` 起点で完走、各 treatment 独立 branch (`treatment-single` / `treatment-dual`) 派生、第 1 系統 (main) は完走済 archive、第 2/第 3 系統は 30th 以降に new branch 派生して 10 round 完走、3 branch 統合で comparison-report v0.2 final 集約 (= ablation study 形式 figure 成立)。**§4 A-2.1 entry に branch 戦略 sub-section 新設** = 全 treatment pristine 起点 + 第 1 系統完走済 + 第 2/3 系統 new branch 派生 + Level 6 events 別 file/sub-group key 記録 (= confound 回避) + 完走後 3 branch 統合分析 + Spec 6 design.md 最終 state は main の post-Round 10 採用 + 他 2 treatment branch は paper data source archive として delete 禁止 + checkbox tracker に第 1 系統 [x] 完走済 + 第 2/3 系統 [ ] 着手予定明示。本 v1.7 改版自体は v1.6 同様 Level 6 記録対象外 (= methodology meta-document)。timeline 文言 (= 9-10 月 paper draft 着手) は v1.6 同様 user 指示「現状維持」遵守、本 v1.7 update は scope (= 3 系統対照実験 state policy 明示) 整合化のみ。
- **v1.6** (2026-05-03 29th セッション末、A-2.3 critical path 外し + §3.7.6 re-impl phase で Claim D primary evidence 代替 = §3.7.6 Level 6 観測統合): 29th 末議論「§3.7.6 で planned されている 3 sample (Phase field / 3D 熱伝導 / Arduino IoT) は既に re-implementation phase を含む = re-impl phase で発生する rework events を Level 6 で観測すれば A-2.3 と等価な Claim D primary evidence を取得可能」を契機に、A-2.3 (= Spec 6 implementation phase) を critical path から外し、Phase B-1.x post-paper supplementary evidence として defer する方針を確定。**§3.6 Level 6 entry update** = 記録範囲を "A-3 + §3.7.6 完走 (= Phase A 終端、v1.6 redefine = §3.7.6 re-impl phase 統合)" に拡張 + **Claim D primary evidence 構成** sub-section 新設 = A-1 (3 spec self-dogfooding、Python、forward-fresh) + §3.7.6 (3 sample external、Julia/C++/Arduino C、reverse-engineered) = **6 spec instance** で構成、A-2.3 を Phase B-1.x defer 明示。**§3.6 baseline 制約 B3 update** = "A-1 vs A-2 within-project" → "A-1 vs §3.7.6 cross-project + cross-language" 比較に置換 (sample size 均衡 3 vs 3、cross-project transfer + 言語 diversity + ground truth-anchored 軸同時取得)、reverse-engineering 5 source bias の Claim D primary metric への影響可能性 caveat 追加 (= forward-fresh A-1 vs reverse-engineered §3.7.6 の sub-group reporting で対処)。**§3.6 checkbox tracker A-2 entry update** = A-2.1 design phase events (= Claim B/C functioning、§5.1.6 整合) + A-2.2 tasks (option) + A-2.3 を Phase B-1.x supplementary defer 明示 + **§3.7.6 Code-derived spec batch re-impl phase entries 新設** = 3 sample × Level 6 観測 (推定 0-3h overhead per sample、Claim D primary evidence の主要 source)。**§3.7.6.1-3 entries に "re-impl phase 中の Level 6 rework events 観測" sub-step 追加** = `spec_id="phase-field|heat-equation-3d|arduino-iot"` + `discovered_phase="impl-mid"` 付与、A-1 vs §3.7.6 sub-group 比較 base として運用。**§3.7.6 全体 cost 見積 update** = 20-32h → 20-41h (= Level 6 観測 0-3h overhead × 3 sample 追加分)、3-6 work day = A-2 完走後 1 calendar 月 batch。**§4 Timeline A-2.3 entry update** = critical path 外し + Phase B-1.x supplementary defer 明示 + Spec 6 implementation 完了時の paper revision supplementary data 後付け運用 (= 論文 draft 着手の前提条件にしない)。本 v1.6 改版自体は v1.5 同様 Level 6 記録対象外 (= methodology meta-document)。timeline 文言 (= 9-10 月 paper draft 着手) は user 指示「現状維持」遵守、本 v1.6 update は scope (= A-2.3 critical path 外し + §3.7.6 で代替) 整合化のみ。
