...
description: design.md / requirements.md / tasks.md 起草直後の同 session orchestrator が validate-design / validate-gap / validate-impl 等 self-review skill を sequential 実行しない。19th 末 A-2 phase 採取軸違反由来
type: feedback
...
**Why:** 19th セッション (2026-05-02) に A-2 phase sub-step 2 = Spec 6 design.md 起草完了 (1146 行) 直後に user 起動 `/kiro-validate-design` で orchestrator (Opus) が同一 session 内 self-review を実行 (3 critical issue identified + NO-GO 判定)、user 指摘で採取軸違反を identified。起草者と reviewer が同一 LLM session = full context shared = self-review contamination で baseline として価値低い、かつ後続 dr-design 等の subagent fresh review 前提を歪める risk (= orchestrator が contamination 結果を反映して artifact を修正すると pristine state が失われ、dual-reviewer Round 1 input が変質)。subagent context には伝達されないため damage は orchestrator level 限定だが、artifact 修正による sequel risk あり (= primary subagent の fresh review 対象が contaminated baseline に変質)。
...
**SSoT**: `data-acquisition-plan.md` v1.4 §7 (commit `f99bdbd`、19th 末新設) + v1.5 で Sub-group analysis 規律と並列 SSoT 化

**関連 memory**: `feedback_dual_reviewer_3_concept_separation.md` (= dual-reviewer skill scope = design phase 専用)、`project_a3_plan_triangulation_defense.md` (= A-3 batch + §3.7.6 Code-derived spec batch も同規律適用範囲)
