...
_v0.7 / 2026-05-04 (40th セッション末、A-2.1 partial completion = treatment=single 系統 10 round 完走 = A-2.1 2/3 終端反映 = §1 readiness 表 update (Claim A/B/C 強化 = treatment=single 単独 detection evidence 追加) + 新規 §4.7 A-2.1 partial completion evidence (treatment=single、5 sub-section = 累計 metric / Round 別 escalate 出現率 / 観察 pattern / Sub-group analysis 規律 / branch 双方向対比) + §7.1 残 work update (treatment=single 完走、treatment=dual 残のみ反映) = data-acquisition-plan v1.8 + evidence-catalog v0.11 整合化)_

_v0.8 / 2026-05-04 (45th セッション末、A-2.1 completion = treatment=dual 系統 10 round 完走 = **A-2.1 全 3 系統完走 = 3 系統対照実験終端** = §1 readiness 表 update (Claim A/B/C/D の ablation evidence 整備完了 = 3 系統比較 final 集約 prerequisite 確立) + 新規 §4.8 A-2.1 completion evidence (treatment=dual、5 sub-section = 累計 metric / adversarial 独立検出補完 5 度目連続再現 / 観察 pattern / Sub-group analysis 規律 / 3 系統比較参照) + §7.1 残 work update (A-2.1 全完走、comparison-report v0.3 final 集約への移行明示) = data-acquisition-plan v1.9 + evidence-catalog v0.12 整合化)_

_v0.6 / 2026-05-03 (30th セッション初頭、A-2.1 3 系統対照実験 design.md state policy 明示 = pristine state `285e762` 起点 + 各 treatment 独立 branch 戦略 = data-acquisition-plan v1.7 + evidence-catalog v0.10 整合化、§7.1 残 work branch 戦略明示 + §6 Limitations に internal validity threat acknowledge 追加 = paper rigor 確保のための confounding 排除)_

_v0.5 / 2026-05-03 (29th セッション末、A-2.3 critical path 外し + §3.7.6 re-impl phase で Claim D primary evidence 代替 = data-acquisition-plan v1.6 + evidence-catalog v0.9 整合化、§1 readiness Claim D row update + §1 Validation framing (v) downstream rework signal 構成 update + §6.6 Claim D scope 構成 update + §7.1 残 work A-2.3 defer 明示 + §7.4 §3.7.6 Level 6 観測明示)_
...
_注 (= 2026-05-07 追記、論文化計画統合に伴う命名対応): SES 2026 (= 5/29 投稿締切) 提出向けの作業群は、別文書の論文化計画 (= paper-submission-plan v1.0 §4) で F (= 形式定義) / Q (= 品質整備) / P (= 論文入力整備) / S (= 起草) / R (= 改訂) の 5 系統に整理されている。本文書中の Phase A 内の作業区分 (= A-1 / A-2 / A-3 / §3.7.6 等) は実証研究の段階区分を指し、F/Q/P/S/R は SES 投稿準備の作業分類で、両者は階層が異なるため共存する。本文書は P-8 (= preliminary-paper-report v0.9、5/12 想定) で命名対応を本文に正式統合する予定。_
...
### 45th 末時点の readiness (v0.8 update、A-2.1 全 3 系統完走 = 3 系統対照実験終端反映)

| 主張 | evidence 充足度 | 備考 |
|------|----------------|------|
| Claim A | **十分** (V3 試験運用 + req phase 3 spec + A-2.1 1/3 終端 v4-miss core evidence 17 件 + treatment=single 完走 = primary 単独 detection 累計 46 件 + escalate=true 17 件 + **45th 末 treatment=dual 完走 = adversarial 独立検出補完 5 度目連続再現 = primary 見落とし補完機能の 10 round 連続実証**) | 致命級独立発見 12 件 + disagreement 17+ 件 + Phase 1 同型 17 度 + A-2.1 dual+judgment 累計 v4-miss core 17 件 + treatment=single Round 1-10 = primary 単独で 46 件検出 + escalate=true 17 件 + **treatment=dual Round 1-10 = adversarial 31 件検出 + adversarial 独立補完 19 件 (= primary 単独検出不可 finding を adversarial が補完する機能の 10 round 連続実証) + forced_divergence 14 件 + 5 度目連続再現 (adversarial 独立検出補完系列) = 3 系統 ablation 完成で adversarial 機能寄与 quantify 完了** |
| Claim B | **強化完了** (V4 redo broad 3 phase × 3 spec + Spec 6 1/3 + treatment=single 完走 = ablation primary baseline 確立 + **45th 末 treatment=dual 完走 = 3 系統 ablation 完成 = adversarial 単独機能寄与 quantify**) | 過剰修正比率 V3 50% → V4 4 spec design phase = 81.25% → 58.8% → 40.0% → 33.3% (Spec 6、29th 末) = -47.95pt 累計改善 + treatment=single 63.0% (ablation primary baseline) + **treatment=dual 21.7% (13/60) = adversarial layer 追加で 63.0% → 21.7% = -41.3pt (= adversarial 単独機能寄与 quantify 完了) = dual+judgment 33.3% との -11.6pt 差分 = judgment layer 追加効果 (adversarial のみより悪化 = judgment が過剰採用を抑制して correctness を向上させる adversarial の機能を一部 neutralize か、あるいは adversarial 単独の検出品質が higher であることの ablation evidence)** = 3 系統 ablation figure 成立 |
| Claim C | **強化完了** (6 spec instance × 2 phase + Spec 6 1/3 + treatment=single 完走 = forward Adjacent Sync 規律遵守 10 round 全件実証 + 既存節拡張 vs 新節新設同型処置 pattern 確立 + **45th 末 treatment=dual 完走 = adversarial 二層構成での forward Adjacent Sync 規律遵守 10 round 全件実証 + cross-spec 整合 Round 8 で adversarial が 6/6 全採用 = adversarial 主導の cross-spec 整合能力実証**) | design phase -47.95pt (4 spec) + tasks phase -23.8pt (3 spec) + escalate 解決手段 5 path 確立 + fatal_pattern hits 5 件 (dual+judgment) + treatment=single 累計: forward Adjacent Sync 規律 5 件全件 forward sync + 後続 → 先行改版要請 0 件 (10 round 連続実証) + 既存節拡張 vs 新節新設同型処置 pattern 確立 + **treatment=dual 累計: forward Adjacent Sync 規律遵守完全実証 (10 round 連続) + adversarial 独立補完が 19/40 events (47.5%) = adversarial layer なしでは primary 単独 miss になっていた high-value finding 群の構造的補完実証 + forced_divergence partially_robust 混在 (R-spec-6-1/8/9) + conclusion_holds=no (R-spec-6-39) = adversarial 機能の質的多様性 evidence = 3 系統 ablation で adversarial / judgment 各 layer の機能寄与が分離定量化完了** |
| Claim D | **中間 evidence 維持 + 構成 update** (= A-1 strong + §3.7.6 で代替予定、v0.5 update) | **Claim D primary evidence 構成 (v0.5 redefine)**: A-1 全 implementation phase (3 spec self-dogfooding、Python、forward-fresh、現状 0 events strong) + §3.7.6 re-implementation phase (3 sample external、Julia/C++/Arduino C、reverse-engineered、A-3 batch 内取得予定) = 6 spec instance / A-2.3 を critical path から外し Phase B-1.x supplementary defer / A-2.1 design phase events (main 44 + treatment-single 17 = 61 件累計) = **Claim D primary ではなく Claim B/C functioning evidence** (= §6.6 で 2 evidence 種別 disambiguate) / sub-group reporting = forward-fresh A-1 vs reverse-engineered §3.7.6 で別 metric 計算必須 |

### core figure 1-3 + ablation evidence の readiness

**A-2.1 完走 = 論文 core evidence (ablation) 取得完了** (= Spec 6 30 review session × 3 系統対照実験 = treatment=single vs treatment=dual vs treatment=dual+judgment の 3 系統 ablation data 完全取得)。残 = `metric_extractor.py` + `figure_data_generator.py` + `phase_b_judgment.py` 実行 → comparison-report v0.3 final 集約 (= ablation figure 成立 + 3 系統 ablation 数値 final 確定)。
...
- (v) downstream rework signal (= Level 6 既存 plan、Claim D primary evidence、time-deferred validity、**v0.5 redefine = A-1 全 implementation phase (3 spec self-dogfooding) + §3.7.6 re-implementation phase (3 sample reverse-engineered) = 6 spec instance、A-2.3 Spec 6 implementation phase は Phase B-1.x supplementary defer**)
...
### §4.7 A-2.1 Spec 6 Design phase partial completion evidence (= treatment=single 全 10 ラウンド完走、40th 末新規、v0.7 追加)

**status (40th 末)**: A-2.1 Spec 6 Design phase の **treatment=single 全 10 ラウンド完走 = A-2.1 2/3 終端**。残 = treatment=dual (Round 1-10) = 10 review session、推定 1 month batch。31st-40th セッション累計 21 commits、40th 末 endpoint = `33e1a12` (treatment-single branch、push 済 = `86e7760..33e1a12`)。pristine state `285e762` 起点 + new branch `treatment-single` 派生 = data-acquisition-plan v1.7 §A-2.1 branch 戦略遵守。

**§4.7.1 累計 metric (treatment=single、10 ラウンド aggregate、40th 末)**

| metric | 値 | treatment=dual+judgment 比較 |
|--------|----|------------------------------|
| primary detection 累計 | 46 | 48 (-2、実質同等、Round 1 = 3 vs 0 が主差) |
| 採用累計 (= must_fix or should_fix apply) | 17 | 21 must_fix + 25 should_fix (-29、user 採用基準は同等近傍) |
| skip 累計 (= do_not_fix) | 29 | 23 (+6) |
| escalate=true 累計 | 17 | 4 (escalate path 適用 = +13、primary 単独で escalate 自己発生) |
| **過剰修正比率 (= skip / detect)** | **63.0%** (29/46) | **33.3%** (23/69) = **+29.7pt 悪化** |
| Level 6 events 累計 | 17 | 44 (-27、treatment=single 修正 17 件 vs dual+judgment 44 件 = adversarial / judgment 補完による追加修正) |
| design.md 行数増加 | +63 行 (1150 → 1213) | +116 行 (1150 → 1266) (-53、treatment=dual+judgment は judgment による追加採用で行数増) |

= **ablation primary baseline 確立 (Claim B strong evidence)**: treatment=single 過剰修正比率 63.0% は primary 単独 = adversarial / judgment 補完なし系統で、treatment=dual+judgment 33.3% との -29.7pt 差分が adversarial + judgment 各 layer 機能寄与の quantify base = 第 3 系統 treatment=dual 完走後の ablation 完成で adversarial 単独機能 vs judgment 単独機能を分離可能。

**§4.7.2 Round 別 escalate=true 出現率 (= 観点 axis 別 pattern observation、40th 末新規)**
...
| 4 | 責務境界 | 1/5 | 80% | c 文書 vs 実装不整合 (responsibility_boundary 直接 hit) |
| 5 | 失敗モード+観測 | 0/5 | **100%** | b 構造的不均一 |
| 6 | concurrency / timing | 3/5 | 40% | b+c 多軸 (concurrency_safety + responsibility_boundary 多軸同時 hit) |
...
= **observation (Claim A 補強 candidate)**: primary subagent が観点 axis 別に escalate 出現 pattern を異にする independent evidence:

- **規範範囲 (Round 1)** = escalate 必須条件 5 種に直接含まれる → 0% suppress (primary が単独で escalate 必要と判定)
- **構造軸 (Round 2 一貫性、Round 5 失敗モード+観測)** = 67-100% suppress (primary が「構造改善」を bias_self_suppression default で抑制)
- **実装軸 (Round 3)** = 100% suppress (primary が「impl phase 委譲可」で全件 INFO 化)
- **責務境界系 (Round 4 / 6 / 7 / 8 / 9 / 10)** = 40-80% suppress (= primary が単独で escalate 判定する困難観点 group、escalate 必須条件 5 種に直接含まれるが他観点と多軸同時 hit のため suppress 寄り)

論文 narrative source 候補: 「primary 単独 review は観点軸により escalate 出現が systematic に異なる」= adversarial / judgment 補完がない系統での primary bias self-management 限界の evidence、treatment=dual / treatment=dual+judgment との比較で「層追加で escalate handling がどう変化するか」の ablation narrative 構築可能。

**§4.7.3 観察 pattern (Claim C 補強 candidate、40th 末新規)**

- **既存節拡張 vs 新節新設同型処置 pattern 確立**: Round 6 P-1 (Concurrency Boundary 節新設) + Round 7 P-2 (Security Considerations 節 4 軸構造化既存節拡張) + Round 9 P-1 (Cross-spec Contract Tests 節新設) + Round 9 P-2 (TDD 規律節新設) + Round 10 P-2 (Atomic Write 実装パターン sub-heading 新設) = **新節新設 4 件累計** + Round 10 P-1 (Monitoring 既存節拡張 = 2 bullet 追加) = 処置 type 多様性確認 = primary 単独でも構造的修正の 2 type (新節 vs 既存節) を文脈に応じて選択する evidence
- **forward Adjacent Sync 規律遵守完全実証**: Round 1-10 全件 forward sync (= 本 spec design.md 内自己完結化採択) + 後続 → 先行改版要請 0 件 + dominated 除外案厳格運用 = memory `feedback_adjacent_sync_direction.md` 規律遵守完全実証 = primary 単独でも spec 間 adjacency 構造を維持する evidence
- **fatal_patterns hit 1 件**: Round 7 P-1 path_traversal hit、Round 4 P-1 cosmetic skip → Round 7 P-1 escalate 反転 = **観点切替で fresh detect = 採取軸保護観察事例** = primary 単独でも観点軸切替で fatal_pattern 検出能力が活性化する evidence
- **採取軸保護観察事例 (= 補完深掘り深化系列)**: Round 4 P-1 → Round 7 P-1 → Round 9 P-3 + Round 4 P-4 → Round 7 P-1 → Round 8 P-2 → Round 10 P-1 = caller-callee consistency / responsibility_boundary 軸の独立 detect 系列拡張 = primary が観点切替で同 pattern を異なる軸から fresh detect する evidence

**§4.7.4 Sub-group analysis 規律遵守 (data-acquisition-plan v1.5 §7 整合)**
...
- ✅ branch 物理分離 + sub-group key 分離 double protection 実証: main の 44 events (第 1 系統 dual+judgment) と treatment-single branch の 17 events (第 2 系統 single) が完全分離維持 = confounding 排除 paper rigor 確保
- ⏸️ §3.7.6 reverse-engineered batch との sub-group 比較 = §3.7.6 着手後 (= A-3 batch)

**§4.7.5 detail reference**

詳細な Round 別 metric / finding rationale / dominated 除外案 / Adjacent Sync 規律遵守事例 / 観察 pattern 解析は `evidence-catalog.md` v0.11 §5.2.1.6 に systematic 記録済。本 §4.7 は論文 narrative 用の summary、論文 figure / table 直接 input。

---

### §4.8 A-2.1 completion evidence (= treatment=dual 全 10 ラウンド完走、45th 末新規、v0.8 追加)

**status (45th 末)**: A-2.1 Spec 6 Design phase の **treatment=dual 全 10 ラウンド完走 = A-2.1 全 3 系統完走 = 3 系統対照実験終端**。41st-45th セッション累計、45th 末 endpoint = `1b15138` (Round 10 修正 commit) / `86cdc5f` (log commit)。pristine state `285e762` 起点 + new branch `treatment-dual` 派生 = data-acquisition-plan v1.7 §A-2.1 branch 戦略遵守。

**§4.8.1 累計 metric (treatment=dual、10 ラウンド aggregate)**

| metric | 値 | treatment=single 比較 | treatment=dual+judgment 比較 |
|--------|----|-----------------------|------------------------------|
| primary detection 累計 | 29 | 46 (-17) | 48 (-19) |
| adversarial detection 累計 | 31 | N/A (なし) | 54 (-23) |
| total detect (P+A) | 60 | 46 (+14) | 69 (-9) |
| 採用 (rework events) | 40 | 17 (+23) | 44 (-4) |
| skip | 13 | 29 (-16) | 23 (-10) |
| **過剰修正比率 (= skip / detect)** | **21.7%** (13/60) | **63.0%** (+41.3pt) | **33.3%** (+11.6pt) |
| Level 6 events 累計 | 40 件 | 17 件 (+23) | 44 件 (-4) |
| escalate=true 累計 | 0 件 | 17 件 (-17) | 4 件 (-4) |
| forced_divergence 累計 | 14 件 | N/A | N/A |
| design.md 行数増加 | +52 行 (1150→1202) | +63 行 | +116 行 |

= **3 系統 ablation 数値確定**: treatment=single 63.0% → treatment=dual+judgment 33.3% → treatment=dual 21.7%。treatment=dual の過剰修正比率が treatment=dual+judgment より低い (= adversarial layer 単独で judgment layer 込みより積極的修正採用 = judgment が一部採用を抑制する構造) = adversarial / judgment 各 layer の機能分離 evidence。

**§4.8.2 Round 別 escalate 出現率**

treatment=dual では judgment skip のため dev_log の `judgment_escalate_count` は全 10 Round で 0。adversarial `forced_divergence` が judgment の代替チェック機能を担い、rework_log 新形式エントリ (R-spec-6-32〜37) には escalate_condition field (normative_scope / implementation_impossibility / responsibility_boundary 等) が記録されている。escalate 判定は user が直接行う構成 (= treatment=dual の設計)。

| Round | 観点 | detect (P+A) | adversarial 独立補完 | forced_div | Level 6 events |
|-------|------|-------------|---------------------|-----------|----------------|
| 1 | 規範範囲確認 | 2 | 1 | 1 | 3 |
| 2 | 一貫性 | 4 | 1 | 2 | 4 |
| 3 | 実装可能性+アルゴリズム+性能 | 5 | 2 | 3 | 4 |
| 4 | 責務境界 | 7 | 2 | 1 | 5 |
| 5 | 失敗モード+観測 | 7 | 2 | 1 | 4 |
| 6 | concurrency / timing | 7 | 4 | 1 | 6 |
| 7 | security | 9 | 0 (merged 2 件) | 1 | 5 |
| 8 | cross-spec 整合 | 8 | 5 | 1 | 6 |
| 9 | test 戦略 | 5 | 1 | 2 | 2 |
| 10 | 運用 | 6 | 1 | 1 | 1 |

= **observation**: adversarial 独立補完は cross-spec 整合 (Round 8) で最多 (5 件全件 adversarial 主導採用) = adversarial layer が cross-spec contract 検出に特に有効な evidence。security (Round 7) では P+A の merged 統合が 2 件発生 = P+A 同型重複の高頻度ラウンド。

**§4.8.3 観察 pattern (Claim A/C 補強 candidate)**

- **adversarial 独立検出補完 5 度目連続再現**: R-spec-6-1 (Round 1 normative_scope) → R-spec-6-38 (Round 9 4 度目) → R-spec-6-40 (Round 10 5 度目) = primary が single では miss した finding を adversarial が単独補完するパターンが 10 round を通じて連続再現 = adversarial layer の primary 補完機能の構造的実証
- **forced_divergence partially_robust + no 混在**: R-spec-6-1 (partially_robust) / R-spec-6-8 (partially_robust) / R-spec-6-9 (partially_robust) / R-spec-6-39 (conclusion_holds=no) = adversarial が forced_divergence で primary の判断を partially_robust (= 部分的に成立) または完全否定 (no) に再評価する機能の多様性確認
- **cross-spec 整合 Round 8 全件 adversarial 主導**: R-spec-6-32〜37 の 6 entries 中 5 件が adversarial 検出 + 1 件 primary 検出 (P-1) = adversarial layer が隣接 spec interface contract 検出に特化した補完能力を持つことの direct evidence
- **P+A 横断同型重複 4 度累計**: R-spec-6-15 (Round 4) / R-spec-6-17 (Round 5) / R-spec-6-21 (Round 6) / R-spec-6-27 (Round 7) = primary と adversarial が独立文脈から同一 finding を検出する convergent detection pattern の 4 度連続再現 = inter-rater reliability proxy evidence

**§4.8.4 Sub-group analysis 規律遵守 (data-acquisition-plan v1.5 §7 整合)**
...
- ✅ branch 物理分離 + sub-group key 分離 double protection 実証: main の 44 events (第 1 系統 dual+judgment) + treatment-single branch の 17 events (第 2 系統 single) + treatment-dual branch の 40 events (第 3 系統 dual) が完全分離維持 = confounding 排除 paper rigor 確保 = 3 系統 ablation figure 成立
- ⏸️ §3.7.6 reverse-engineered batch との sub-group 比較 = §3.7.6 着手後 (= A-3 batch)

**§4.8.5 detail reference**

詳細な Round 別 metric / finding rationale / dominated 除外案 / Adjacent Sync 規律遵守事例 / 観察 pattern 解析は `evidence-catalog.md` v0.12 §5.2.1.7 に systematic 記録済。本 §4.8 は論文 narrative 用の summary、論文 figure / table 直接 input。
...
- **A-2.1 3 系統対照実験 design.md state policy (v0.6 新設、internal validity threat 対処)**: 30 review session の input design.md state は **全 treatment が pristine state `285e762` (= 19th sub-step 2 = Spec 6 design.md 起草直後) 起点で独立 branch 完走** (= data-acquisition-plan v1.7 §3.6 B4 整合)。累積修正 state を共有して各 treatment Round 1-10 を回すと「prior treatment による事前修正」と「current treatment 単独 effect」が交絡 = V4 各 layer の機能寄与を quantify 不能 + Claim B/C primary evidence の paper rigor 致命的弱体化 (= self-referential bias = internal validity threat)。pristine state 起点 + 各 treatment 独立 branch (`treatment-single` / `treatment-dual`) 派生で confounding 排除、3 系統間の検出件数 / 採択率 / 過剰修正比率 / Level 6 events / wall-clock の直接比較性が保証され ablation study 形式 figure 成立。**論文 Limitations 記述方針**: 本 state policy を Methodology section に明示 + Threats to Validity section の internal validity 項で confounding control の措置として記述 (= 各 treatment 独立 branch 派生 + Level 6 events 別記録 + 3 branch 統合分析の手順を再現性確保のため明記)。
...
- **§3.7.6 re-implementation phase = Claim D primary evidence の主要 source (v0.5 新規)**: 3 sample (Phase field / 3D 熱伝導 / Arduino IoT、A-3 batch 内取得予定) の re-implementation phase で発生する rework events を Level 6 schema で観測 = **Spec 6 を含まない外部 project + 言語 diversity (Julia/C++/Arduino C) + reverse-engineered spec source** で構成、A-1 と合わせて **6 spec instance for Claim D primary evidence**
...
- **A-2.3 Spec 6 implementation phase = Phase B-1.x supplementary defer (v0.5 update from "30th 以降取得")**: critical path から外し、Spec 6 implementation 完了時点で paper revision 用補強 data として後付け運用、論文 draft 着手の前提条件にしない (data-acquisition-plan v1.6 + evidence-catalog v0.9 §5.1.6 整合)

**論文 framing 必須 (v0.4 強化 / v0.5 update = 6 spec instance + sub-group analysis、論文 reviewer 想定批判への direct rebuttal)**:
...
- **Claim D primary evidence の構成 (v0.5 redefine)**:
  - A-1 (= 3 spec self-dogfooding、Python、forward-fresh-spec、現状 0 events strong)
  - **§3.7.6 (= 3 sample external project、Julia/C++/Arduino C、reverse-engineered-spec、A-3 batch 内取得予定)**
  - = 6 spec instance accumulated Claim D primary evidence (= sample size 倍増 + cross-project transfer + 言語 diversity + ground truth-anchored 補強)
...
- **A-2.3 Spec 6 implementation phase events の論文 framing (v0.5 新規)**:
  - **Phase B-1.x post-paper supplementary evidence として位置付け** = paper revision 用補強 data
  - **Claim D primary evidence の構成 (= A-1 + §3.7.6) には含めない** (= 論文 draft 着手の前提条件にしない、cost / wall-clock 1 month 待ちを critical path から外す)
...
- **論文 Limitations section 明記推奨 (v0.5 拡張)**:
  ...
  - (4) **"Claim D primary evidence is composed of forward-fresh-spec sub-group (A-1) and reverse-engineered-spec sub-group (§3.7.6); reverse-engineering 5 source bias is acknowledged and addressed via sub-group reporting"** (v0.5 新規) = forward / reverse 別 sub-group reporting で reverse-engineering bias 5 source (spec completeness / convergent thinking / design choice rationalization / edge case asymmetry / implementation detail leakage) を direct acknowledge
...
- ~~**A-2.1 第 2 系統 = treatment=single 全 10 round 完走**~~ (31st-40th 末完走 ✅、treatment-single branch endpoint `33e1a12`、push 済、§4.7 partial completion evidence):
  - 過剰修正比率 63.0% (29/46) = ablation primary baseline 確立、treatment=dual+judgment 33.3% 比 +29.7pt
  - Round 別 escalate 出現率 (Round 1 0% → Round 10 40% suppress、観点 axis 別 pattern observation = 規範範囲 0% / 実装 100% / 責務境界系 40-80%)
  - Level 6 events 17 件累計 + forward Adjacent Sync 規律 10 round 連続実証 + 既存節拡張 vs 新節新設同型処置 pattern 確立 (新節新設 4 件 + 既存節拡張 1 件) + fatal_pattern 1 hit (Round 7 P-1 path_traversal、Round 4 P-1 → Round 7 P-1 escalate 反転 = 採取軸保護観察事例)
- **A-2.1 残 1/3** (41st 以降、推定 1 month batch、v0.7 update = treatment=single 完走反映): treatment=dual (Round 1-10) = 10 review session = single → dual で adversarial 効果分離 (= dual+judgment 33.3% との比較で judgment 単独機能寄与 quantify) = 論文 figure ablation primary source 完成。**branch 戦略 (data-acquisition-plan v1.7 §3.6 B4 + §4 A-2.1 整合)**: 第 3 系統 = new branch `treatment-dual` で pristine state `285e762` (= 19th sub-step 2 起草直後) 起点に Round 1-10 完走、各 treatment 独立 branch + Level 6 events 別 sub-group key 記録 (= confound 回避)、A-2.1 完走後に 3 branch 統合分析で comparison-report v0.2 final 集約 + ablation figure 成立、Spec 6 design.md 最終 state は main の post-Round 10 (`f6bac54`) 採用 + 他 2 treatment branch (`treatment-single` + `treatment-dual`) は paper data source archive として保持 (delete 禁止)
- **A-2.2 Spec 6 Tasks phase** (option、推定 数時間-1 日、A-2.1 完走後): V4 ad-hoc 補助 evidence、tasks phase 4 spec 目拡張
- **A-2.3 Spec 6 Impl phase** (v0.5 update = critical path 外し、Phase B-1.x supplementary defer): Spec 6 implementation 完了時点で paper revision 用補強 data として後付け運用、論文 draft 着手の前提条件にしない (= Claim D primary evidence は §3.7.6 で代替取得)
- **A-2 終端統合分析** (v0.5 update = A-2.3 を含めず A-2.1+A-2.2 完走時): A-1 vs §3.7.6 cross-project 比較 (= primary) + A-1 vs A-2.3 (= supplementary、Phase B-1.x 後付け) + Claim D 主張可否判断 + Spec 6 全 phase approve = Rwiki v2 全 8 spec design phase approve
...
### 7.1.bis A-2.1 全完走 + comparison-report v0.3 final 集約 (45th 末確定)

45th 末で treatment=dual 全 10 ラウンド完走 = **A-2.1 全 3 系統完走確定**。3 系統 ablation data 完全取得済 (= main 44 events / treatment-single 17 events / treatment-dual 40 events)。

**残 work**: `metric_extractor.py` + `figure_data_generator.py` + `phase_b_judgment.py` 実行 → comparison-report v0.3 final 集約 = 3 系統間 ablation figure 生成 + adversarial / judgment 各 layer 機能寄与 quantify final + A-2.1 完走宣言。3 branch は paper data source archive として保持 (delete 禁止 = data-acquisition-plan v1.7 §3.6 B4 規律維持)。
...
- **§3.7.6 Code-derived spec batch** (= v0.3 新規、3 sample reverse-engineered、v0.5 update = 累計 cost 20-32h → 20-41h で Level 6 観測 overhead 0-9h 追加、批判 1 ground truth-anchored 補強 + 批判 2 domain abstraction gap 解消 + 批判 9 言語 generalization defense + **Claim D primary evidence の主要 source として位置付け** = A-2.3 critical path 外しの代替):
  - **§3.7.6.1** Phase field 法 (compact、Julia or C++、cost 6-10h + Level 6 観測 0-3h、reverse-engineering feasibility 検証 sample = 着手順序最先行)
  - **§3.7.6.2** 3D 熱伝導方程式 + 複雑モデル生成 (cost 10-14h + Level 6 観測 0-3h、real-world numerical project rich 化)
  - **§3.7.6.3** Arduino IoT センサ (cost 4-8h + Level 6 観測 0-3h、embedded systems failure mode cover)
  - **re-impl phase Level 6 観測 (v0.5 新設、Claim D primary evidence の主要 source)**: 3 sample 全件 re-implementation phase で発生する rework events を Level 6 schema で観測、`spec_id="phase-field|heat-equation-3d|arduino-iot"` + `discovered_phase="impl-mid"` 付与、A-1 (= 3 spec self-dogfooding、forward-fresh) との sub-group 比較で 6 spec instance accumulated Claim D evidence 構成
...
- **v0.8** (2026-05-04 45th セッション末、treatment=dual 系統 10 round 完走 = **A-2.1 全 3 系統完走 = 3 系統対照実験終端** 反映): treatment=dual branch (`treatment-dual`) 全 10 ラウンド完走 (= 41st-45th セッション累計、endpoint `1b15138` / log `86cdc5f`、pristine state `285e762` 起点)。**冒頭 status update**: v0.8 改版 + A-2.1 全 3 系統完走確定。**§1 readiness 表 update (4 row 全件 update)**: Claim A 強化完了 (adversarial 独立補完 5 度目連続再現 = treatment=dual 10 round 全件実証) + Claim B 強化完了 (3 系統 ablation 完成 = treatment=dual 過剰修正比率 21.7% 確定 + adversarial 単独機能寄与 quantify = 63.0% → 21.7% = -41.3pt) + Claim C 強化完了 (3 系統 ablation で adversarial / judgment 各 layer 機能寄与 分離定量化完了) + Claim D 構成 update 不要 (= A-2.1 design phase events = Claim B/C functioning evidence 継続)。**core figure + ablation readiness update** = A-2.1 完走で論文 core evidence (ablation data) 取得完了、残 = comparison-report v0.3 final 集約スクリプト実行のみ。**新規 §4.8 A-2.1 completion evidence (treatment=dual)** = §4.8.1 累計 metric (3 系統比較 table = single 63.0% / dual+judgment 33.3% / dual 21.7%) + §4.8.2 Round 別 adversarial 独立補完 + forced_divergence 出現 + §4.8.3 観察 pattern (adversarial 独立検出補完 5 度目連続再現 / forced_divergence partially_robust + no 混在 / cross-spec Round 8 全件 adversarial 主導 / P+A 横断同型重複 4 度累計) + §4.8.4 Sub-group analysis 規律遵守 + §4.8.5 evidence-catalog v0.12 §5.2.1.7 詳細参照。**§7.1.bis 新設**: A-2.1 全完走 + comparison-report v0.3 final 集約残 work 明示。本 v0.8 改版自体は v0.7 同様 Level 6 記録対象外 (= methodology meta-document)。
- **v0.6** (2026-05-03 30th セッション初頭、A-2.1 3 系統対照実験 design.md state policy 明示 = pristine state 起点 + 各 treatment 独立 branch 戦略 = data-acquisition-plan v1.7 + evidence-catalog v0.10 整合化、paper rigor 確保のための confounding 排除): 30th セッション開始時に user 指摘「single の Round 1 を開始するとき、元になる design.md はどうやって準備するか」を契機に、A-2.1 30 review session の input design.md state policy が SSoT 文書群に明示記述されていない盲点を identified。29th 末まで第 1 系統 (= main、`treatment=dual+judgment`) が pristine state `285e762` から 10 round 累積修正で `f6bac54` に到達、accumulated state を起点に第 2/第 3 系統を回すと「prior treatment による事前修正」と「current treatment 単独 effect」が交絡 = V4 各 layer の機能寄与を quantify 不能 + Claim B/C primary evidence の paper rigor 致命的弱体化 (= self-referential bias = internal validity threat)。**§7.1 残 work A-2.1 残 2/3 entry に branch 戦略 sub-bullet 追加** = 第 2 系統 (`treatment-single`) + 第 3 系統 (`treatment-dual`) が new branch `285e762` 起点完走 + Level 6 events 別 file/sub-group key 記録 + 3 branch 統合分析で comparison-report v0.2 final 集約 + Spec 6 design.md 最終 state は main の post-Round 10 (`f6bac54`) 採用 + 他 2 treatment branch は paper data source archive として保持 (delete 禁止)。**§6.1 sample size に A-2.1 3 系統対照実験 design.md state policy sub-bullet 追加** = pristine state 起点 + 各 treatment 独立 branch 戦略 + confounding 排除根拠 + 論文 Limitations 記述方針 (= Methodology section に state policy 明示 + Threats to Validity section の internal validity 項で confounding control 措置記述、再現性確保)。本 v0.6 改版自体は v0.5 同様 Level 6 記録対象外 (= methodology meta-document)。timeline 文言 (= 9-10 月 paper draft 着手) は user 指示「現状維持」遵守、本 v0.6 update は scope (= 3 系統対照実験 state policy 明示) 整合化のみ。**scope 制限注記**: §4.1-§4.5 = 14th 末 historical fix 維持方針継続、A-2.1 完全終端 (= 30 session) 後の v0.7 で systematic update 予定。
- **v0.5** (2026-05-03 29th セッション末、A-2.3 critical path 外し + §3.7.6 re-impl phase で Claim D primary evidence 代替 = data-acquisition-plan v1.6 + evidence-catalog v0.9 整合化): 29th 末議論「§3.7.6 で planned されている 3 sample (Phase field / 3D 熱伝導 / Arduino IoT) は既に re-implementation phase を含む = re-impl phase で発生する rework events を Level 6 で観測すれば A-2.3 と等価な Claim D primary evidence を取得可能」採用、A-2.3 を critical path から外し Phase B-1.x post-paper supplementary evidence として defer する方針確定。**§1 readiness 表 Claim D row update** = 中間 evidence + 構成 update = A-1 strong + §3.7.6 で代替予定、6 spec instance for Claim D primary evidence 明示。**§1 Validation framing (v) downstream rework signal 構成 update** = "A-1 全 implementation phase (3 spec self-dogfooding) + §3.7.6 re-implementation phase (3 sample reverse-engineered) = 6 spec instance、A-2.3 Spec 6 implementation phase は Phase B-1.x supplementary defer" 明示。**§6.6 Claim D の precise scope 構成 update** = 3 evidence 種別 disambiguate (Claim D primary = A-1 + §3.7.6 / Claim B/C functioning = A-2.1 design 44 events / Phase B-1.x supplementary = A-2.3 Spec 6 impl 後付け) + 論文 framing 必須 update (= sub-group reporting forward (A-1) vs reverse (§3.7.6) + reverse-engineering 5 source bias acknowledge) + 論文 Limitations section 4 件記述方針 (= primary metric 定義 + out-of-scope 明示 + design phase events を Claim B/C functioning として framing + sub-group analysis 規律明示)。**§7.1 残 work A-2.3 update** = critical path 外し、Phase B-1.x supplementary defer 明示 + A-2 終端統合分析を A-2.1+A-2.2 完走時 (= A-2.3 を含めず) に変更。**§7.4 §3.7.6 entries update** = 累計 cost 20-32h → 20-41h (= Level 6 観測 overhead 0-9h 追加分) + re-impl phase Level 6 観測明示 + Claim D primary evidence の主要 source として位置付け = A-2.3 critical path 外しの代替。本 v0.5 改版自体は v0.4 同様 Level 6 記録対象外 (= methodology meta-document)。timeline 文言 (= 9-10 月 paper draft 着手) は user 指示「現状維持」遵守、本 v0.5 update は scope (= A-2.3 critical path 外し + §3.7.6 で代替) 整合化のみ。**scope 制限注記**: §4.1-§4.5 = 14th 末 historical fix 維持方針継続、A-2.1 完全終端 (= 30 session) 後の v0.6 で systematic update 予定。
