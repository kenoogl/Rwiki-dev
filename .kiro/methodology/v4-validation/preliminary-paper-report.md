...
| Claim D | **中間 evidence 維持 + 構成 update** (= A-1 strong + §3.7.6 で代替予定、v0.5 update) | **Claim D primary evidence 構成 (v0.5 redefine)**: A-1 全 implementation phase (3 spec self-dogfooding、Python、forward-fresh、現状 0 events strong) + **§3.7.6 re-implementation phase (3 sample external、Julia/C++/Arduino C、reverse-engineered、A-3 batch 内取得予定)** = 6 spec instance / **A-2.3 を critical path から外し Phase B-1.x supplementary defer** (= Spec 6 implementation 完了時点で paper revision 用補強 data として後付け、論文 draft 着手の前提条件にしない) / A-2.1 design phase 44 events = **Claim D primary ではなく Claim B/C functioning evidence** (= §6.6 で 2 evidence 種別 disambiguate) / sub-group reporting = forward-fresh A-1 vs reverse-engineered §3.7.6 で別 metric 計算必須 |
...
- **A-2.1 残 2/3** (30th 以降、推定 1-2 month batch、v0.6 update = branch 戦略明示): treatment=single (Round 1-10) + treatment=dual (Round 1-10) = 20 review session = ablation comparison data 取得 = single → dual で adversarial 効果分離 + dual → dual+judgment で judgment 効果分離 = 論文 figure ablation primary source。**branch 戦略 (v0.6 新設、data-acquisition-plan v1.7 §3.6 B4 + §4 A-2.1 整合)**: 第 2 系統 = new branch `treatment-single` で pristine state `285e762` (= 19th sub-step 2 起草直後) 起点に Round 1-10 完走、第 3 系統 = new branch `treatment-dual` で同様完走、各 treatment 独立 branch + Level 6 events 別 file/sub-group key 記録 (= confound 回避)、A-2.1 完走後に 3 branch 統合分析で comparison-report v0.2 final 集約 + ablation figure 成立、Spec 6 design.md 最終 state は main の post-Round 10 (`f6bac54`) 採用 + 他 2 treatment branch は paper data source archive として保持 (delete 禁止)
