...
- 差し戻し区分の件数内訳が記録されているのは実装適合レビュー（implementation-conformance）のファイルのみ。原典では旧表記 A／B／C／D（新表記の I-0／I-2／I-3／I-4 に対応、本論文§3.3 参照）で記録されている。要件・設計・タスクの各個別レビューは must-fix／should-fix／leave-as-is で集計し、新表記の段内吸収（R-0／D-0／A-0）と他機能波及の二軸で本抽出に変換している。
...
- **段内吸収（absorption）**：その段の成果物（要件書／設計書／タスク書／実装）に編集を加えることで解消する所見の件数。
  - 要件・設計・タスク段：must-fix と should-fix の合計を「修正適用による段内吸収」、leave-as-is を「記録のみの段内吸収」として併記。新表記では R-0／D-0／A-0 に対応。
  - 実装適合段：新表記 I-0（task-local／fix-in-current-branch、旧表記 A）の件数。
- **上位 handback**：上位フェーズへ差し戻された所見の件数と差し戻し先。
  - 要件・設計・タスク段：原典の「帰属別分類」節に「要件への差し戻し」「他 spec 設計への波及」等として明示された件数。新表記では R-1（要件→意図）、D-1（設計→要件）、D-2（設計→意図）、A-1（タスク→設計）、A-2（タスク→要件）、A-3（タスク→意図）に相当。
  - 実装適合段：新表記 I-2（reopen-design、旧表記 B）、I-3（reopen-requirements、旧表記 C）、I-4（reopen-intent、旧表記 D）の件数。実装段からタスク段への 1 段差し戻しに相当する **I-1** は原典記録には独立記号が無く、I-0 に圧縮されて記録されている（本論文§3.3 の新表記と§5 の限界を参照）。
...
### 0.3 implementation 段から **タスク段への差し戻し記号 I-1** が原典に欠落（本論文§3.3／§5／§6.3 参照）

実装適合レビューの原典は handback 区分を A（task-local／fix-in-current-branch）／B（reopen-design）／C（reopen-requirements）／D（reopen-intent）の 4 種で記録している。本論文§3.3 で導入した二次元表記では、A＝I-0、B＝I-2、C＝I-3、D＝I-4 に対応する。実装段から 1 段上のタスク段への差し戻し（**I-1**）に相当する独立記号は元ログに存在しない。

実態としては、実装適合レビューが「タスク差分の追記で解決可能」と判断したものはすべて旧 A（新表記 I-0、実装内吸収）に分類されている。タスク書自体の改版を要した事例はあるが（例：実行系の前回 finding 11＝回帰基盤不在、自己改善の Task 9 決定的テスト不在）、それらも実装の小修正波の中で吸収されており、独立した I-1 区分として記号化されていない。本論文では「記号が欠落していた」事実を §5 第五の限界として記録し、次のサイクル以降で全レビュー段にわたるレビュー記録テンプレートと機械抽出コードを新表記に基づいて再整備する（本論文§6.3 「新表記に基づく実装の再整備」、実装案の策定そのものが今後の課題）。
...
- **段内吸収（absorption）**：I-0=8件（旧表記 A、Finding 1〜7, 9。`:38` `:49` `:60` `:71` `:82` `:93` `:104` `:126`）。task-local／fix-in-current-branch／fix-before-next-feature で消化される所見群。
- **上位 handback**：
  - I-2=1件（旧表記 B）：Finding 8 → reopen-design（`:115`, `:144`, `:148`）。受け皿は §1.3。
  - I-3=0（旧表記 C）、I-4=0（旧表記 D）。I-1（実装→タスク段の差し戻し）は原典に独立記号がなく I-0 に圧縮されている（§0.3）。
- **他機能波及**：0件。
...
- **段内吸収（absorption）**：I-0=1件（旧表記 A、新規 P3、Finding 1＝`finding.schema.json` トップレベル `x-deferred` 注記欠落、`:66`）。
- **上位 handback**：I-2=0（旧表記 B）、I-3=0（旧表記 C）、I-4=0（旧表記 D）（`:88`–`:92`, `:94`）。I-1（実装→タスク段）は原典に独立記号なし。
- **他機能波及**：0件。
...
- **段内吸収（absorption）**：I-0=7件（旧表記 A、Finding 1, 3, 4, 7, 8, 10, 11）。
  - 注：本文 `:194` は「A=5件」（新表記 I-0=5件相当）と記すが、同行括弧内に7件を列挙し本文数値と一致しない。確定根拠（複数の独立出典が I-0=7／I-2=4 を支持）：(1) disposition summary（`:187`–`:190`）で reopen-design＝Finding 2/5/6/9（I-2 群4件）、残り7件が非 reopen、(2) §2.3 設計差し戻しが finding 2/5/6/9 を I-2 群4件と明示、(3) git コミット名 `bea2dbeb`「手戻りA7/B4」（旧表記）、(4) 後続 §2.6 が「前回 finding 11件（A7/B4）」と参照（`implementation-conformance-review-2026-05-19.md:133`）。事実として、正は I-0=7／I-2=4、`:194` 本文の「A=5件」は原文の内部記載不整合。
- **上位 handback**：
  - I-2=4件（旧表記 B）：Finding 2/5/6/9 → reopen-design（`:188`）。受け皿は §2.3。
  - I-3=0（旧表記 C）、I-4=0（旧表記 D）。I-1（実装→タスク段）は原典に独立記号なし。
- **他機能波及**：0件。
...
- **段内吸収（absorption）**：I-0=3件（旧表記 A、Finding 1〜3、すべて P3、`:107`、各 Finding `:60` `:72` `:84`）。
- **上位 handback**：I-2=0（旧表記 B）、I-3=0（旧表記 C）、I-4=0（旧表記 D）（`:103`）。I-1（実装→タスク段）は原典に独立記号なし。
- **他機能波及**：0件。
...
- **段内吸収（absorption）**：I-0=10件（旧表記 A、`:178`、各 Finding `:56`–`:155`, `:174`）。
- **上位 handback**：I-2=0（旧表記 B）、I-3=0（旧表記 C）、I-4=0（旧表記 D）（`:174`–`:175`）。「reopen-design／requirements／intent 該当なし」。I-1（実装→タスク段）は原典に独立記号なし。
...
- **段内吸収（absorption）**：I-0=1件（旧表記 A、Finding 1＝旧 v1 残存 writer 3件の mkpath 欠落、`:90`, `:67`, `:86`）。
- **上位 handback**：I-2=0（旧表記 B）、I-3=0（旧表記 C）、I-4=0（旧表記 D）（`:86`）。I-1（実装→タスク段）は原典に独立記号なし。
- **他機能波及**：0件。
...
- **段内吸収（absorption）**：I-0=3件（旧表記 A、`:111`、各 Finding `:64` `:76` `:88`）。
- **上位 handback**：I-2=0（旧表記 B）、I-3=0（旧表記 C）、I-4=0（旧表記 D）（`:107`）。I-1（実装→タスク段）は原典に独立記号なし。
- **他機能波及**：0件。
...
- **段内吸収（absorption）**：I-0=0（旧表記 A、finding ゼロ、`:86`）。
- **上位 handback**：I-2=0（旧表記 B）、I-3=0（旧表記 C）、I-4=0（旧表記 D）（`:86`）。I-1（実装→タスク段）は原典に独立記号なし。
- **他機能波及**：0件。
...
- **段内吸収（absorption）**：I-0=10件（旧表記 A、`:182`、各 Finding `:60`–`:159`, `:178`）。
- **上位 handback**：I-2=0（旧表記 B）、I-3=0（旧表記 C）、I-4=0（旧表記 D）（`:178`）。I-1（実装→タスク段）は原典に独立記号なし。
- **他機能波及**：0件。
...
- **段内吸収（absorption）**：I-0=0（旧表記 A、finding ゼロ、`:86`）。
- **上位 handback**：I-2=0（旧表記 B）、I-3=0（旧表記 C）、I-4=0（旧表記 D）（`:82`）。I-1（実装→タスク段）は原典に独立記号なし。
- **他機能波及**：0件。
...
- **段内吸収（absorption）**：I-0=5件（旧表記 A、Finding 1, 2, 3, 5, 6、`:139`, `:142`、各 Finding `:62`–`:117`）。
- **上位 handback**：
  - I-2 候補=1件（旧表記 B 候補）：Finding 4（design 小節 1.2 行スキーマと authority-map 実装の乖離、利用者判断で I-0 吸収可（旧 A 吸収）、`:97`, `:136`）。実際の処理は §6.9 で I-0 吸収＝design 不変。
  - I-3=0（旧表記 C）、I-4=0（旧表記 D）。I-1（実装→タスク段）は原典に独立記号なし。
...
- **段内吸収（absorption）**：I-0=0（旧表記 A、finding ゼロ、`:87`）。
- **上位 handback**：I-2=0（旧表記 B）、I-3=0（旧表記 C）、I-4=0（旧表記 D）（`:87`, `:82`–`:83`）。前回 Finding 4 の reopen-design 候補は利用者判断で I-0 吸収（旧 A 吸収、authority-map 4列化＋パーサ追従）が選択され、design 小節 1.2 不変のまま解消＝reopen 不発。I-1（実装→タスク段）は原典に独立記号なし。
- **他機能波及**：0件。
- **判定**：「現行承認仕様へ適合（GO 可）。前回 finding 6件（P1=2／P2=2／P3=2、handback 旧表記 A=5＋B 候補1＝新表記 I-0=5＋I-2 候補1）は全6件解消を独立確認。これにて6機能の実装適合フェーズの最後の機能が GO 可」（`:88`, `:121`）。
...
- 実装適合段の handback 構造（再実装前）：
  - foundation：I-0=8、I-2=1、I-3=0、I-4=0
  - runtime：I-0=7、I-2=4、I-3=0、I-4=0
  - evaluation：I-0=10、I-2=0、I-3=0、I-4=0
  - self-improvement：I-0=3、I-2=0、I-3=0、I-4=0
  - paper-interface：I-0=10、I-2=0、I-3=0、I-4=0
  - governance：I-0=5、I-2 候補=1（I-0 吸収で消化）、I-3=0、I-4=0
  - I-1（実装→タスク段の差し戻し）は 6 機能とも原典に独立記号がなく I-0 に圧縮（§0.3）
...
- 差し戻し区分の件数（実装適合段では旧表記 A／B／C／D、本抽出では新表記 I-0／I-2／I-3／I-4 に変換）と §4 metric snapshot は実装適合レビューにのみ存在し、要件・設計・タスクの個別レビューは must-fix／should-fix／leave-as-is で集計される。フェーズ横断で同一指標による定量比較はできない（フェーズで記録語が異なる）。これは限界として論文に明記すべき素材（論文化方針§4「限界・脅威を明記」）。
- 実装段からタスク段への差し戻し記号 **I-1** が原典に欠落していた（§0.3、本論文§3.3／§5／§6.3 参照）。次のサイクル以降で新表記に基づきレビュー記録テンプレートと機械抽出コードを再整備する。
- 原文内部の記述上の不整合：
  - 実行系 再実装前 `implementation-conformance-review-2026-05-18.md:194` 本文「A=5件」（新表記 I-0=5件相当）は同行括弧内列挙7件・disposition summary・git `bea2dbeb`・後続レビューと不一致。正は A7/B4（新表記 I-0=7／I-2=4、§2.5）。
...
- 論文§3.3「手戻り区分（起点フェーズの記号＋深さの数字）」では、新表記の起点フェーズ記号 R／D／A／I と深さ 0／1／2／3／4 を導入し、原典の旧表記 A／B／C／D（新表記の I-0／I-2／I-3／I-4 に対応）との対応を明示する。要件・設計・タスク段の must-fix／should-fix／leave-as-is は新表記の R-0／D-0／A-0 に対応する。両系統の対応関係は「must-fix（修正適用） → 段内吸収（X-0）」「他機能波及あり must-fix → 他機能 spec への記録のみ波及」「タスク段への handback 記号 I-1 は元ログ欠落」と整理する。
...
- 論文§4.6（収束）では、再実装前→再実装後の収束を「件数の減少」だけでなく「handback 区分の変化」で示せる。例：実行系は I-2=4 → I-2=0、評価は I-0=10 → I-0=1、自己改善は I-0=3 → I-0=0、レポートインターフェースは I-0=10 → I-0=0、ワークフロー管理は I-0=5＋I-2 候補1 → I-0=0。
...
- 実装段からタスク段への差し戻し記号 **I-1** を、レビュー記録テンプレートと機械抽出コードに導入する作業は次のサイクル以降の宿題（本論文§6.3「新表記に基づく実装の再整備」）。
