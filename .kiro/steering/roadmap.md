...
**v2 redesign 議論との関係**（2026-04-24 追記）: Rwiki v2 統合仕様書 Draft v0.5 の §10.11 で Scenario 28（Vault スキーマ破壊的変更のマイグレーション）として位置付け、v2 MVP に含めない先送り方針で合意。

### multi-user-collaboration（将来拡張）

**v2 redesign 議論から抽出**（2026-04-24、`.kiro/drafts/rwiki-v2-consolidated-spec.md` §10.9）:

- Rwiki v2 は**単独ユーザー前提**で設計される
- 複数人による共有 Vault 運用は将来拡張として roadmap に保留
- 関連シナリオ:
  - **Scenario 29**: Dirty tree と並行操作のガバナンス（複数セッション・並行 CLI 実行時のロック・タイムスタンプ管理）
  - **Scenario 31**: Vault ポータビリティ（マルチデバイス、Git リモート共有、権限モデル）

着手タイミング: 個人利用の範囲を超えた共有要求（チーム Wiki としての運用）が実際に発生した時点。
