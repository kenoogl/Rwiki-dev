...
- V4 attempt 1 (archive `archive/v4-redo-attempt-1-v3-scope` ):
...
2. **draft.md V4 整合化** (適用 spec が draft 持つ場合): draft v0.3 (本 dual-reviewer-draft.md) 整合の / / / / / / / 改訂
3. **requirements.md V4 整合化**: 全 Req AC が V4 protocol §1 機能を AC レベルで明示

## 案 3 広義 redo の Step 構造 (  )

- Step 0: brief V4 整合化 + draft V4 整合化
- Step 1-3: foundation / design-review / dogfeeding requirements V4 整合化
- Step 4: 各 spec の Stage 1-4 (= 各 spec req review の 4 stage) を V4 protocol 下で実施 (-)
- Step 5: cross-spec review 統合改版 (12 implication 全処理 = apply 8 / 自動解消 3 / resolved 2 / cosmetic 1)
- Step 6: 中間 comparison-report 起草 
- approve + main 統合 + audit 
...
- archive 操作: `archive/{prefix}-{description}-{date}` branch + tag で完全保全 (V3 endpoint = `archive/v3-foundation-design--session`、V4 attempt 1 = `archive/v4-redo-attempt-1-v3-scope`、V4 redo broad merged = `archive/v4-redo-broad-merged-2026-05-01`)

## 関連 reference

- V4 attempt 1 archive: branch `archive/v4-redo-attempt-1-v3-scope`
- V4 redo broad endpoint: main
- comparison-report.md (req phase V4 redo broad evidence)
- evidence-catalog.md §2 (V4 attempt 1) + §3 (V4 redo broad)
- 関連 memory: `feedback_review_v4_necessity_judgment.md` (V4 protocol 確定経緯)
