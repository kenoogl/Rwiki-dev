---
name: 説明文体規律 = 全 user 応答で説明文体 + 1 検出 1 turn (確定 + 5 度連続失敗)
...
user 確定。「常に説明文体」は保証できない (= training data default は dense academic 文体、説明文体は意識的切替必要)。user 指摘 → 私書き直し前提で運用、structural 補助で引力弱化。

## 違反 example (= 最新、Round 8、5 度連続失敗)
