---
name: 周辺 work は subagent dispatch で実施 (= 主 context 保持、確定)
description: 主 context を work-specific に保つため、TODO 更新 / memory 整理 / status report 等の周辺 work は subagent dispatch で isolated context 内で実施。 context compaction experiment で stage_4 採用根拠
type: feedback
...
- methodology 4 文書 SSoT update (= preliminary-paper-report 各 round 完走時の 反映)
...
context compaction experiment 5 trial で stage_4 (= 主 context 縮約 + subagent dispatch pattern 意識化) が baseline (= stage_0) から規律違反 raw 件数 100% 削減 (= 8 → 0) で最 effective と判定。driver = context 量だけでなく構造 (= active 必読 vs 参照層分離 + subagent pattern 意識化)、4 機構分析の機構 3 (= 規律意識の分散) が最大と推定。
